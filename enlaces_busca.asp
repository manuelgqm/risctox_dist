<%
	...
	idpagina = 675	'--- página listado enlaces Vig. Tecnológica (sólo para registrar estadísticas)
	'----- Registrar la visita
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "<a href='enlaces1.asp'>Inicio</a></p>"
              				   else
              				     response.write "<p class=campo>&nbsp;</p>"
              				   end if %>
				
					<% 
					   if request("busca_ambito_te")<>"" then
					   	sql = "SELECT nombre FROM ENL_TEMAS WHERE id="&request("busca_ambito_te")
			   		   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
			   		   	set objRecordset = OBJConnection.Execute(sql)
			   		   	if not objRecordset.eof then texto_ambito_te = objRecordset("nombre")
			   		   	'response.write "busca_ambito_te="&request("busca_ambito_te")&"<br>"
			   		   end if
			   		   
			   		   if request("busca_ambito_am")<>"" then
					   	sql = "SELECT nombre FROM ENL_TEMAS WHERE id="&request("busca_ambito_am")
			   		   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
			   		   	set objRecordset = OBJConnection.Execute(sql)
			   		   	if not objRecordset.eof then texto_ambito_am = objRecordset("nombre")
			   		   	'response.write "busca_ambito_am="&request("busca_ambito_am")&"<br>"
			   		   end if
			   		   
			   		   if request("busca_industria")<>"" then
					   	sql = "SELECT nombre FROM ENL_TEMAS WHERE id="&request("busca_industria")
			   		   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
			   		   	set objRecordset = OBJConnection.Execute(sql)
			   		   	if not objRecordset.eof then texto_industria = objRecordset("nombre")
			   		   	'response.write "busca_industria="&request("busca_industria")&"<br>"
			   		   end if
			   		   
					   'sql = "SELECT DISTINCT ENL_ENLACES.Id,ENL_ENLACES.afiliacion,ENL_ENLACES.titulo,ENL_ENLACES.url FROM ENL_ENLACES LEFT JOIN ENL_CLASIFICACION ON ENL_ENLACES.Id = ENL_CLASIFICACION.enlace LEFT JOIN ENL_TEMAS ON ENL_CLASIFICACION.tema = ENL_TEMAS.id WHERE (ENL_ENLACES.titulo LIKE '%"&request("busca_texto")&"%' OR ENL_ENLACES.afiliacion LIKE '%"&request("busca_texto")&"%') "
					   sql = "SELECT DISTINCT ENL_ENLACES.Id,ENL_ENLACES.afiliacion,ENL_ENLACES.titulo,ENL_ENLACES.url FROM ENL_ENLACES LEFT JOIN ENL_CLASIFICACION ON ENL_ENLACES.Id = ENL_CLASIFICACION.enlace LEFT JOIN ENL_CLASIFICACION AS ENL_CLASIFICACION1 ON ENL_ENLACES.Id = ENL_CLASIFICACION1.enlace LEFT JOIN ENL_CLASIFICACION AS ENL_CLASIFICACION2 ON ENL_ENLACES.Id = ENL_CLASIFICACION2.enlace "
					   sql = sql & "WHERE (ENL_ENLACES.titulo LIKE '%"&request("busca_texto")&"%' OR ENL_ENLACES.afiliacion LIKE '%"&request("busca_texto")&"%') "
					   if request("busca_ambito_te")<>"" then sql = sql & " AND ENL_CLASIFICACION.tema="&request("busca_ambito_te")
					   if request("busca_ambito_am")<>"" then sql = sql & " AND ENL_CLASIFICACION1.tema="&request("busca_ambito_am")
					   if request("busca_industria")<>"" then sql = sql & " AND ENL_CLASIFICACION2.tema="&request("busca_industria")
					   'sql = sql & " ORDER BY ENL_CLASIFICACION.orden"
			   		   'response.write sql
			   		   set objRecordset = Server.CreateObject ("ADODB.Recordset")
			   		   'set objRecordset = OBJConnection.Execute(sql)
					...
					<p class="titulo2">Resultados de la búsqueda en la base de datos de enlaces:</p>
					<p class=texto"><b>Texto:</b>&nbsp;<%=request("busca_texto")%><br>
					<b>Ámbito territorial:</b>&nbsp;<%=texto_ambito_te %><br>
					<b>Aspectos ambientales:</b>&nbsp;<%=texto_ambito_am %><br>
					<b>Ámbito sectorial:</b>&nbsp;<%=texto_industria %>
					<p class=texto">
					<% if num_resultados=0 then %>
					No hay ningún enlace con estos parámetros. Vuelve a buscar con otros parámetros.
					<% else %>
					Hay <%=num_resultados%> enlaces. Pulsa sobre uno de ellos para visitarlo.
					<% end if %>
					</p>
					...
					<tr><td class="texto" valign="top" align="right"><a href="abreenlacer.asp?idenlace=<%=objRecordset("Id")%>" target="_blank"><img src="imagenes/ico_puntito.gif" valign="top" border=0></a></td><td class="texto" bgcolor="#<%=color%>"><b><%=objRecordset("afiliacion")%></b><br><a href="abreenlacer.asp?idenlace=<%=objRecordset("Id")%>" target="_blank"><%=objRecordset("titulo")%><br><a href="abreenlacer.asp?idenlace=<%=objRecordset("Id")%>" target="_blank"><%=objRecordset("url")%></td></tr>
					<% i=-1*i
					   objRecordset.movenext
					   loop %>
					</table>
					...
					<% if num_resultados=0 then %><p align="center"><input type="button" class="boton" value="imprimir" onclick="print()"></p><% end if %>
					<P class="titulo2">Buscador de enlaces:</p>
					<form name="buscador" action="enlaces_busca.asp" method="POST">
					...
					<tr><td class="campo" align="left" nowrap>Ámbito territorial:&nbsp;<select name="busca_ambito_te" class="campo">
						...
						<% sql = "SELECT numeracion FROM ENL_TEMAS WHERE id=561"
						set objRecordset = Server.CreateObject ("ADODB.Recordset")
			   		   	set objRecordset = OBJConnection.Execute(sql)
						...
						<option value="">- cualquiera -</option>
						<% sql = "SELECT numeracion FROM ENL_TEMAS WHERE id=563"
						set objRecordset = Server.CreateObject ("ADODB.Recordset")
			   		   	set objRecordset = OBJConnection.Execute(sql)
			   		   	if not objRecordset.eof then numeracion = objRecordset("numeracion")
						sql = "SELECT id,nombre FROM ENL_TEMAS WHERE numeracion LIKE '" & numeracion & "%' AND numeracion<>'" & numeracion & "' ORDER BY numeracion"
					...
					<tr><td class="texto" align="left" colspan=2 nowrap>Ámbito sectorial:&nbsp;<select name="busca_industria" class="campo">
						<option value="">- cualquiera -</option>
						<% sql = "SELECT numeracion FROM ENL_TEMAS WHERE id=562"
						set objRecordset = Server.CreateObject ("ADODB.Recordset")
			   		   	set objRecordset = OBJConnection.Execute(sql)
			   		   	if not objRecordset.eof then numeracion = objRecordset("numeracion")
						sql = "SELECT id,nombre FROM ENL_TEMAS WHERE numeracion LIKE '" & numeracion & "%' AND numeracion<>'" & numeracion & "' ORDER BY numeracion"
