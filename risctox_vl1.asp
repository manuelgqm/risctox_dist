<% response.redirect "dn_risctox_vl1.asp" %>
<%
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "<a href=risctox1.asp>Inicio</a>&nbsp;&gt;&nbsp;Límites de exposición profesional: Valores Límite Ambientales</p>"
              				   else
              				     response.write "<p class=campo>&nbsp;</p>"
              			   end if %>
				
				<p class=titulo3>RISCTOX: Límites de exposición profesional: Valores Límite Ambientales</p>

				<form name="form_buscar" action="risctox_vl1.asp">
				<table class="tabla3" width="90%" align="center">
				  <tr>
					<td class="subtitulo3" nowrap align="right">Buscador de sustancias con valores límite ambientales:&nbsp;</td>
...
<%				buscar = request("buscar")
				...
				if request("ordenacion")="" then 
					ordenacion = "RQ_SUSTANCIAS.nombre"
				else
					ordenacion = request("ordenacion")
				end if
				if right(ordenacion,4)<>"DESC" then texto_ord = " DESC"
					
				sql = "SELECT RQ_SUSTANCIAS.NOMBRE, RQ_SUSTANCIAS_VL1.CAS, RQ_SUSTANCIAS_VL1.SUSTANCIA, RQ_SUSTANCIAS_VL1.vlaed1, RQ_SUSTANCIAS_VL1.vlaed2, RQ_SUSTANCIAS_VL1.vlaec1, RQ_SUSTANCIAS_VL1.vlaec2, RQ_SUSTANCIAS_VL1.notas "
				sql = sql & "FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_VL1 ON RQ_SUSTANCIAS.CAS = RQ_SUSTANCIAS_VL1.CAS "
				if buscar<>"" then sql = sql & "WHERE (RQ_SUSTANCIAS_VL1.SUSTANCIA LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_VL1.CAS LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.nombre LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.cas LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.cee LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.rd LIKE '%"&buscar&"%') "
				...
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
		   	   	...
		   	   	if registros=0 then 
		   	   		...
		   	   		response.write "<tr><td class=texto colspan=10>No hay sustancias con este criterio de búsqueda</td></tr>"
		   	   	else 
		   	   	...
		   	   	%>
				  <tr>
				  	<td class="subtitulo3" valign="top">
				  		<a href="risctox_vl1.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS.nombre<%=texto_ord%>">Sustancias (<%=registros%>)</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS.nombre DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS.nombre" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3" valign="top">
				  		<a href="risctox_vl1.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_VL1.cas<%=texto_ord%>">CAS</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_VL1.cas DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_VL1.cas" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3" valign="top">
				  		<a href="risctox_vl1.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_VL1.vlaed1<%=texto_ord%>">VLA-ED</a>
				  		<% if ordenacion="RQ_SUSTANCIAS_VL1.vlaed1 DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_VL1.vlaed1" then response.write "<img src=imagenes/arriba.gif>" %>(ppm)
				  	</td>
				  	<td class="subtitulo3" valign="top">
				  		<a href="risctox_vl1.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_VL1.vlaed2<%=texto_ord%>">VLA-ED</a>
				  		<% if ordenacion="RQ_SUSTANCIAS_VL1.vlaed2 DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_VL1.vlaed2" then response.write "<img src=imagenes/arriba.gif>" %>(mg/m<sup>2</sup>)
				  	</td>
				  	<td class="subtitulo3" valign="top">
				  		<a href="risctox_vl1.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_VL1.vlaec1<%=texto_ord%>">VLA-EC</a>
				  		<% if ordenacion="RQ_SUSTANCIAS_VL1.vlaec1 DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_VL1.vlaec1" then response.write "<img src=imagenes/arriba.gif>" %>(ppm)
				  	</td>
				  	<td class="subtitulo3" valign="top">
				  		<a href="risctox_vl1.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_VL1.vlaec2<%=texto_ord%>">VLA-EC</a>
				  		<% if ordenacion="RQ_SUSTANCIAS_VL1.vlaec2 DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_VL1.vlaec2" then response.write "<img src=imagenes/arriba.gif>" %>(mg/m<sup>2</sup>)
				  	</td>
				  	<td class="subtitulo3" valign="top">
				  		<a href="risctox_vl1.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_VL1.notas<%=texto_ord%>">NOTAS</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_VL1.notas DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_VL1.notas" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  </tr>
<%				objRecordset.movefirst
				...
				<tr>
					...
					<td class="celda_risctox" align="center"><%=formato(objRecordset("vlaed1"),15)%>&nbsp;</td>
					<td class="celda_risctox" align="center"><%=formato(objRecordset("vlaed2"),15)%>&nbsp;</td>
					<td class="celda_risctox" align="center"><%=formato(objRecordset("vlaec1"),15)%>&nbsp;</td>
					<td class="celda_risctox" align="center"><%=formato(objRecordset("vlaec2"),15)%>&nbsp;</td>
					<td class="celda_risctox"><%=objRecordset("notas")%>&nbsp;</td>
				</tr>
<%				objRecordset.movenext
				reg = reg+1
				loop
				end if %>							
				<tr><td class=celda_risctox colspan=10>&nbsp;</td></tr>
<% 				if objRecordset.Pagecount>1 then%>
				<tr><td class=texto colspan=10 align="center">Hay <%=registros%> sustancias que cumplen los criterios de búsqueda. Se muestran sólo <%=registrosporpagina%> por página.</td></tr>
				<tr><td class=texto colspan=10 align="center">
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="risctox_vl1.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="risctox_vl1.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
<%				end if %>				
				...
				<input type="hidden" name="campo3" value="vlaed1">
				<input type="hidden" name="campo4" value="vlaed2">
				<input type="hidden" name="campo5" value="vlaec1">
				<input type="hidden" name="campo6" value="vlaec2">
				<input type="hidden" name="campo7" value="notas">
				<input type="hidden" name="nombre1" value="nombre/sustancia">
				<input type="hidden" name="nombre2" value="cas">
				<input type="hidden" name="nombre3" value="VLA-ED (ppm)">
				<input type="hidden" name="nombre4" value="VLA-ED (mg/m2)">
				<input type="hidden" name="nombre5" value="VLA-EC (ppm)">
				<input type="hidden" name="nombre6" value="VLA-EC (mg/m2)">
				<input type="hidden" name="nombre7" value="NOTAS">
				
				<input type="hidden" name="sql" value="<%=sql%>">
				<input type="hidden" name="buscar" value="<%=buscar%>">
				<input type="hidden" name="titulo" value="Límites de exposición profesional: Valores Límite Ambientales">
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='risctox_vl1.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
</script>
