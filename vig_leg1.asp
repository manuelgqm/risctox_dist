				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write titulo&"<a href=index.asp?idpagina=571>Vigilancia legislativa</a>&nbsp;&gt;&nbsp;Inicio</p>"
              				   else
              				     response.write "<p class=campo>&nbsp;</p>"
              				   end if %>
				
					<p class="titulo2">Elige un sector industrial para consultar la legislación referente a él o usa el buscador:</p>
					...
					<% sql = "SELECT idtipo,nombre FROM ECO_VIG_LEG_TIPOS ORDER BY idtipo"
			   		   ...
			   		   set objRecordset = OBJConnection.Execute(sql)
			   		   do while not objRecordset.eof %>
					<tr><td class="texto"><img src="imagenes/ico_industria.gif" border="0" align="absmiddle">&nbsp;<a href="vig_leg2.asp?idtipo=<%=objRecordset("idtipo")%>"><%=objRecordset("nombre")%></a></td></tr>
					<% objRecordset.movenext
					   loop %>
					</table>
