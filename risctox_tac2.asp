<% response.redirect "dn_risctox_tac2.asp" %>
<%
	...
	if session("id_ecogente")="" then response.redirect "acceso.asp?pagina_esp=risctox_tac2.asp"
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "<a href=risctox1.asp>Inicio</a>&nbsp;&gt;&nbsp;Sustancias peligrosas agua Alemania</p>"
              				   else
              				     response.write "<p class=campo>&nbsp;</p>"
              			   end if %>
				
				<p class=titulo3>RISCTOX: Sustancias peligrosas agua Alemania</p>

				<form name="form_buscar" action="risctox_tac2.asp">
				<table class="tabla3" width="90%" align="center">
				  <tr>
					<td class="subtitulo3" nowrap align="right">Buscador de sustancias peligrosas agua Alemania:&nbsp;</td>
				  ...
				  <tr>
					<td class="texto">(según Lista alemana)&nbsp;</td>
...
<%				buscar = request("buscar")
				...
				'sql = "SELECT RQ_SUSTANCIAS.*,RQ_SUSTANCIAS_TAC2.campo3 FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_TAC2 ON RQ_SUSTANCIAS.CAS=RQ_SUSTANCIAS_TAC2.CAS "
				'sql = sql & "WHERE RQ_SUSTANCIAS.nombre<>'' AND (nombre LIKE '%"&buscar&"%' OR sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_TAC2.cas LIKE '%"&buscar&"%' OR cee LIKE '%"&buscar&"%' OR rd LIKE '%"&buscar&"%') ORDER BY "&ordenacion
				
				sql = "SELECT RQ_SUSTANCIAS.NOMBRE, RQ_SUSTANCIAS_TAC2.campo3 as sustancia, RQ_SUSTANCIAS_TAC2.CAS, RQ_SUSTANCIAS_TAC2.campo5 "
				sql = sql & "FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_TAC2 ON RQ_SUSTANCIAS.CAS = RQ_SUSTANCIAS_TAC2.CAS "
				if buscar<>"" then sql = sql & "WHERE (RQ_SUSTANCIAS_TAC2.campo3 LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_TAC2.cas LIKE '%"&buscar&"%' OR nombre LIKE '%"&buscar&"%' OR sinonimos LIKE '%"&buscar&"%' OR cee LIKE '%"&buscar&"%' OR rd LIKE '%"&buscar&"%') "
				...
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
				  ...
				  <tr>
				  	<td class="subtitulo3">
				  		<a href="risctox_tac2.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS.nombre<%=texto_ord%>">Sustancias (<%=registros%>)</a>&nbsp;
				  	...
				  	<td class="subtitulo3">
				  		<a href="risctox_tac2.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_TAC2.cas<%=texto_ord%>">CAS</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_TAC2.cas DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_TAC2.cas" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3">
				  		<a href="risctox_tac2.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_TAC2.campo5<%=texto_ord%>">Clasificación</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_TAC2.campo5 DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_TAC2.campo5" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  </tr>
<%				objRecordset.movefirst
				...
				<tr>
					...
					if objRecordset("campo5")="nwg" then response.write "no peligrosa para aguas"
					if objRecordset("campo5")="1" then response.write "baja peligrosidad para aguas"
					if objRecordset("campo5")="2" then response.write "peligrosa para aguas"
					if objRecordset("campo5")="3" then response.write "elevada peligrosidad para aguas"
					%>
...
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="risctox_tac2.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="risctox_tac2.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
<%				end if %>				
				...
				<input type="hidden" name="campo1" value="RQ_SUSTANCIAS.nombre">
				<input type="hidden" name="campo2" value="RQ_SUSTANCIAS_TAC2.cas">
				<input type="hidden" name="campo3" value="campo4">
				<input type="hidden" name="nombre1" value="nombre/sustancia">
				<input type="hidden" name="nombre2" value="cas">
				<input type="hidden" name="nombre3" value="Clasificación">
				<input type="hidden" name="sql" value="<%=sql%>">
				<input type="hidden" name="buscar" value="<%=buscar%>">
				<input type="hidden" name="titulo" value="Sustancias peligrosas agua Alemania">
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='risctox_tac2.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
</script>
