<% response.redirect "dn_risctox_tac.asp" %>
<%
	...
	if session("id_ecogente")="" then response.redirect "acceso.asp?pagina_esp=risctox_tac.asp"
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "<a href=risctox1.asp>Inicio</a>&nbsp;&gt;&nbsp;Sustancias de toxicidad acuática según Directiva de aguas</p>"
              				   else
              				     response.write "<p class=campo>&nbsp;</p>"
              			   end if %>
				
				<p class=titulo3>RISCTOX: Sustancias de toxicidad acuática según Directiva de aguas</p>

				<form name="form_buscar" action="risctox_tac.asp">
				<table class="tabla3" width="90%" align="center">
				  <tr>
					<td class="subtitulo3" nowrap align="right">Buscador de sustancias de toxicidad acuática:&nbsp;</td>
				  ...
				  <tr>
					<td class="texto">(según Directiva de aguas)&nbsp;</td>
...
<%				buscar = request("buscar")
				...
				if request("ordenacion")="" then 
					ordenacion = "RQ_SUSTANCIAS_TAC.nombre"
				else
					ordenacion = request("ordenacion")
				end if
				if right(ordenacion,4)<>"DESC" then texto_ord = " DESC"
					
				'sql = "SELECT RQ_SUSTANCIAS.*,RQ_SUSTANCIAS_TAC.campo3 FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_TAC ON RQ_SUSTANCIAS.CAS=RQ_SUSTANCIAS_TAC.CAS "
				'sql = sql & "WHERE RQ_SUSTANCIAS.nombre<>'' AND (nombre LIKE '%"&buscar&"%' OR sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_TAC.cas LIKE '%"&buscar&"%' OR cee LIKE '%"&buscar&"%' OR rd LIKE '%"&buscar&"%') ORDER BY "&ordenacion
				
				sql = "SELECT RQ_SUSTANCIAS.NOMBRE, RQ_SUSTANCIAS_TAC.CAS, RQ_SUSTANCIAS_TAC.NOMBRE AS SUSTANCIA, RQ_SUSTANCIAS_TAC.CE, RQ_SUSTANCIAS.RD "
				sql = sql & "FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_TAC ON RQ_SUSTANCIAS.CAS = RQ_SUSTANCIAS_TAC.CAS "
				if buscar<>"" then sql = sql & "WHERE (RQ_SUSTANCIAS_TAC.nombre LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_TAC.CAS LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.nombre LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.cas LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.cee LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.rd LIKE '%"&buscar&"%') "
				...
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
				  ...
				  <tr>
				  	<td class="subtitulo3">
				  		<a href="risctox_tac.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_TAC.nombre<%=texto_ord%>">Sustancias (<%=registros%>)</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_TAC.nombre DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_TAC.nombre" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3">
				  		<a href="risctox_tac.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_TAC.cas<%=texto_ord%>">CAS</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_TAC.cas DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_TAC.cas" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3">
				  		<a href="risctox_tac.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS.rd<%=texto_ord%>">RD</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS.rd DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS.rd" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  </tr>
<%				objRecordset.movefirst
				...
				<tr>
					...
					<td class="celda_risctox" nowrap><%=formato(objRecordset("RD"),15)%>&nbsp;</td>
				</tr>
<%				objRecordset.movenext
				reg = reg+1
				loop
...
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="risctox_tac.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="risctox_tac.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
<%				end if %>				
				...
				<input type="hidden" name="campo1" value="RQ_SUSTANCIAS_TAC.sustancia">
				<input type="hidden" name="campo2" value="RQ_SUSTANCIAS_TAC.cas">
				<input type="hidden" name="campo3" value="RD">
				<input type="hidden" name="nombre1" value="nombre/sustancia">
				<input type="hidden" name="nombre2" value="cas">
				<input type="hidden" name="nombre3" value="RD">
				<input type="hidden" name="sql" value="<%=sql%>">
				<input type="hidden" name="buscar" value="<%=buscar%>">
				<input type="hidden" name="titulo" value="Sustancias de toxicidad acuática según Directiva de aguas">
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='risctox_tac.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
</script>
