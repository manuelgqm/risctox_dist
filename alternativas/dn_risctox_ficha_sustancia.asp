...
<head>
<title>ECOinformas: Plataforma prevención de riesgo químico</title>
...
<tr>
<td><p class=campo>Est&aacute;s en: <a href="index.asp?idpagina=550">prevención riesgo químico</a> &gt; <a href="dn_risctox_buscador.asp">bbdd risctox</a> &gt; ficha de sustancia</p></td>
...
%>
	...
	%>
		<tr>
			<td class="subtitulo3" align="right" valign="top">
				<a onclick=window.open('ver_definicion.asp?id=83','def','width=300,height=200,scrollbars=yes,resizable=yes') style='cursor:hand'><img src='imagenes/ayuda.gif' width=14 height=14 align='absmiddle' /></a> Sinónimos:
			</td>
			<td class="texto" valign="middle">
				<%=sinonimos%>
			</td>
...
sub bucle_frases_r(byval frases_r)
		' Pasandole las frases R separadas por comas, muestra cada una junto a su descripción
...
<%
		...
		if ((estado_1 <> "") or (vla_ed_ppm_1 <> "") or (vla_ed_mg_m3_1 <> "") or (vla_ec_ppm_1 <> "") or (vla_ec_mg_m3_1 <> "") or (notas_vla_1 <> "") or (estado_2 <> "") or (vla_ed_ppm_2 <> "") or (vla_ed_mg_m3_2 <> "") or (vla_ec_ppm_2 <> "") or (vla_ec_mg_m3_2 <> "") or (notas_vla_2 <> "") or (estado_3 <> "") or (vla_ed_ppm_3 <> "") or (vla_ed_mg_m3_3 <> "") or (vla_ec_ppm_3 <> "") or (vla_ec_mg_m3_3 <> "") or (notas_vla_3 <> "") or (estado_4 <> "") or (vla_ed_ppm_4 <> "") or (vla_ed_mg_m3_4 <> "") or (vla_ec_ppm_4 <> "") or (vla_ec_mg_m3_4 <> "") or (notas_vla_4 <> "") or (estado_5 <> "") or (vla_ed_ppm_5 <> "") or (vla_ed_mg_m3_5 <> "") or (vla_ec_ppm_5 <> "") or (vla_ec_mg_m3_5 <> "") or (notas_vla_5 <> "") or (estado_6 <> "") or (vla_ed_ppm_6 <> "") or (vla_ed_mg_m3_6 <> "") or (vla_ec_ppm_6  <> "") or (vla_ec_mg_m3_6 <> "") or (notas_vla_6 <> "")) then
%>
	<span id="ap2_clasificacion_vla_titulo" class="ficha_titulo_1"><a href="index.asp?idpagina=616"><img src="http://www.istas.net/ecoinformas/web/imagenes/ayuda.gif" align="absmiddle" border="0" /></a>Valores Límite Ambientales <% plegador "secc-vla", "img-vla" %></span>
...
%>
		...
		<p id="ap2_clasificacion_vlb_titulo" class="ficha_titulo_1"><a href="index.asp?idpagina=616"><img src="http://www.istas.net/ecoinformas/web/imagenes/ayuda.gif" align="absmiddle" border="0" /></a>Valores Límite Biológicos <% plegador "secc-vlb", "img-vlb" %></p>
...
%>
		...
		<p id="secc-listanegra" class="texto" style="display:none">Esta sustancia está incluida en la Lista negra de ISTAS ya que es <%=razones%></p>
...
<%
		...
		if esta_en_lista_neurotoxico then ap3_riesgos_tabla("Neurotóxico") end if
...
<%
		if esta_en_lista_tpb then ap3_riesgos_tabla("Tóxica, Persistente y Bioacumulativa") end if
...
%>
	<table class="ficharisctox" width="90%" align="center" border="0" cellpadding="4" cellspacing="0">
   	<tr>
        ...
        <% if ((tipo <> "COV") and (tipo <> "Vertidos") and (tipo <> "LPCIC (EPER Agua)") and (tipo <> "LPCIC (EPER Aire)") and (tipo <> "Residuos Peligrosos") and (tipo <> "Accidentes Graves") and (tipo <> "Emisiones Atmosféricas")) then %>
...
%>
			<fieldset>
				<legend class="subtitulo3"><strong>Según Real Decreto 363/1995</strong></legend>
				<blockquote>
<%
		nivel_cancerigeno_rd = dame_nivel_cancerigeno_rd()
		if (nivel_cancerigeno_rd <> "") then
			response.write "<strong>Nivel cancerígeno:</strong> "&nivel_cancerigeno_rd
%>
...
%>
			<fieldset>
				<legend class="subtitulo3"><strong>Según IARC <a onclick=window.open('ver_definicion.asp?id=<%=dame_id_definicion("IARC")%>','def','width=300,height=200,scrollbars=yes,resizable=yes') style='cursor:hand'><img src='imagenes/ayuda.gif' width=14 height=14 align='absmiddle' border='0' /></a></strong></legend>
<%
...
%>
			<fieldset>
				<legend class="subtitulo3"><strong>Según <%=trim(array_fuentes(i))%> <a onclick=window.open('ver_definicion.asp?id=<%=dame_id_definicion(trim(array_fuentes(i)))%>','def','width=300,height=200,scrollbars=yes,resizable=yes') style='cursor:hand'><img src='imagenes/ayuda.gif' width=14 height=14 align='absmiddle' border='0' /></a></strong></legend>
				<blockquote>	
				<table>
...
%>
			<fieldset>
				<legend class="subtitulo3"><strong>Según SSI (cáncer de mama) <a onclick=window.open('ver_definicion.asp?id=<%=dame_id_definicion("SSI")%>','def','width=300,height=200,scrollbars=yes,resizable=yes') style='cursor:hand'><img src='imagenes/ayuda.gif' width=14 height=14 align='absmiddle' border='0' /></a></strong></legend>
				<blockquote>	
				<table>
...
%>
			<fieldset>
				...
				<%
					...
					response.write "<br /><strong>Nivel mutágeno:</strong> "&nivel_mutageno_rd 
				%>
...
<%
      ...
      if esta_en_lista_sensibilizante then
        response.write "<li class='subtitulo3'>Sensibilizante según Real Decreto 363/1995</li>"
      end if
...
<%
			  ...
			  %>
  				<blockquote>
  					<strong>Categoría:</strong> <%=nivel_reproduccion_rd%>
...
%>
				<tr>
					<td class="subtitulo3" colspan="2">· Según Directiva de Aguas</td>
				</tr>
<%
			end if
...
%>
				<tr>
					<td class="subtitulo3">
						· Según Peligrosas Agua Alemania</td><td><strong>Clasificación</strong>: <%=clasif_mma%>
...
%>
					<tr>
						...
						<td>Sustancia incluida en la <a href="http://www.istas.net/ecoinformas/web/abreenlace.asp?idenlace=2234">Directiva 96/62/CE</a> de 27 de septiembre sobre evaluación y gestión de la calidad del aire ambiente</td>
					</tr>
<%
				end if
%>			
...
%>
					<tr>
						...
						<td>Sustancia que agota la capa de ozono, según <a href="http://www.istas.net/ecoinformas/web/abreenlace.asp?idenlace=2229">Reglamento (CE) 2037/2000</a> del Parlamento Europeo y del Consejo, de 29 de junio de 2000</td>
					</tr>
<%
				end if
%>			
...
%>
				...
				%>
					<tr>
						<td class="subtitulo3" align="right" valign="top">Síntomas:</td><td><%=replace(objRstEnf("sintomas"), vbcrlf, "<br>")%></td>
					</tr>
				<%
					end if
				%>
