<% response.redirect "dn_risctox_vl3.asp" %>
<%
	...
	FUNCTION formato(x,lon)
		if isnull(x) then
			...
			x = ucase(x)
			if len(x)>(lon-3) then x = "<a title='"&x&"' style=text-decoration:none>"&mid(x,1,lon-3)&"...</a>"
			formato = x
		end if
	END FUNCTION
...
%>
			...
			<div id="texto">
				...
				<p class=titulo3>RISCTOX: Límites de exposición profesional: Valores Límite Biológicos</p>

				<form name="form_buscar" action="risctox_vl3.asp">
				<table class="tabla3" width="90%" align="center">
				  <tr>
					<td class="subtitulo3" nowrap align="right">Buscador de sustancias con valores límite biológicos:&nbsp;</td>
...
<%				buscar = request("buscar")
				...
				sql = "SELECT RQ_SUSTANCIAS.NOMBRE, RQ_SUSTANCIAS_VL3.AGENTE, RQ_SUSTANCIAS_VL3.CAS, RQ_SUSTANCIAS_VL3.IB, RQ_SUSTANCIAS_VL3.VLB, RQ_SUSTANCIAS_VL3.MOMENTO_MUESTREO, RQ_SUSTANCIAS_VL3.notas "
				sql = sql & "FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_VL3 ON RQ_SUSTANCIAS.CAS = RQ_SUSTANCIAS_VL3.CAS "
				...
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
				  ...
				  <tr>
				  	<td class="subtitulo3" valign="top">
				  		<a href="risctox_vl3.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS.nombre<%=texto_ord%>">Sustancias (<%=registros%>)</a>&nbsp;
				  	...
				  	<td class="subtitulo3" valign="top">
				  		<a href="risctox_vl3.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_VL3.cas<%=texto_ord%>">CAS</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_VL3.cas DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_VL3.cas" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3" valign="top">
				  		<a href="risctox_vl3.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_VL3.IB<%=texto_ord%>">Indicador Biológico</a>
				  		<% if ordenacion="RQ_SUSTANCIAS_VL3.IB DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_VL3.IB" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3" valign="top">
				  		<a href="risctox_vl3.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_VL3.VLB<%=texto_ord%>">VLB</a>
				  		<% if ordenacion="RQ_SUSTANCIAS_VL3.VLB DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_VL3.VLB" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3" valign="top">
				  		<a href="risctox_vl3.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_VL3.MOMENTO_MUESTREO<%=texto_ord%>">Momento de muestreo</a>
				  		<% if ordenacion="RQ_SUSTANCIAS_VL3.MOMENTO_MUESTREO DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_VL3.MOMENTO_MUESTREO" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3" valign="top">
				  		<a href="risctox_vl3.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_VL3.notas<%=texto_ord%>">Notas</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_VL3.notas DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_VL3.notas" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  </tr>
<%				objRecordset.movefirst
				...
				do while not objRecordset.eof and reg<registrosporpagina
					...
					if objRecordset("nombre")<>"" then
						...
						nombre_actual = objRecordset("agente")
					end if %>
				<tr>
					...
					<td class="celda_risctox" align="center"><%=formato(objRecordset("IB"),15)%>&nbsp;</td>
					<td class="celda_risctox" align="center"><%=formato(objRecordset("VLB"),15)%>&nbsp;</td>
					<td class="celda_risctox" align="center"><%=formato(objRecordset("MOMENTO_MUESTREO"),15)%>&nbsp;</td>
...
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="risctox_vl3.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="risctox_vl3.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
<%				end if %>				
				...
				<input type="hidden" name="campo3" value="ib">
				<input type="hidden" name="campo4" value="vlb">
				<input type="hidden" name="campo5" value="momento_muestreo">
				<input type="hidden" name="campo6" value="notas">
				<input type="hidden" name="nombre1" value="nombre/sustancia">
				<input type="hidden" name="nombre2" value="cas">
				<input type="hidden" name="nombre3" value="Indicador Biológico">
				<input type="hidden" name="nombre4" value="VLB">
				<input type="hidden" name="nombre5" value="Momento Muestreo">
				<input type="hidden" name="nombre6" value="Notas">
				
				<input type="hidden" name="sql" value="<%=sql%>">
				<input type="hidden" name="buscar" value="<%=buscar%>">
				<input type="hidden" name="titulo" value="Límites de exposición profesional: Valores Límite Biológicos">
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='risctox_vl3.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
</script>
