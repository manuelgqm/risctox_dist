...
<%
	...
	if frase_s="todas" then
		sql = "SELECT * FROM dn_risc_frases_s ORDER BY frase"
		...
		for i=0 to Ubound(partes)
			sql = "SELECT * FROM dn_risc_frases_s WHERE frase='S"&partes(i)&"'"
			set objRecordset = Server.CreateObject ("ADODB.Recordset")
			objRecordset.Open sql,objConnection,adOpenKeyset
...
<body>
<table><tr><td class="titulo3">S-Phrases</td></tr>
<tr><td class="campo">Safety advice concerning dangerous substances and preparations</td></tr>
</table>
&nbsp;
<table class="tabla3" width="90%" align="center" height="100%" valign="middle">
<%=texto%>
</table>
<p align="center"><input type="button" class="boton" value="See all S-Phrases" onClick="location.href='busca_frases_s.asp?id=todas'"></p>
</body>
</html>