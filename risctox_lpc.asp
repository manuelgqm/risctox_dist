<% response.redirect "dn_risctox_lpc.asp" %>
<%
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "<a href=risctox1.asp>Inicio</a>&nbsp;&gt;&nbsp;Sustancias (LPCIC)</p>"
              				   else
              				     response.write "<p class=campo>&nbsp;</p>"
              			   end if %>
				
				<p class=titulo3>RISCTOX: Sustancias (LPCIC)</p>

				<form name="form_buscar" action="risctox_lpc.asp">
				<table class="tabla3" width="90%" align="center">
				  <tr>
					<td class="subtitulo3" nowrap align="right">Buscador de sustancias (LPCIC):&nbsp;</td>
...
<%				buscar = request("buscar")
				...
				if request("ordenacion")="" then 
					ordenacion = "RQ_SUSTANCIAS_LPC.SUSTANCIA"
				else
					ordenacion = request("ordenacion")
				end if
				if right(ordenacion,4)<>"DESC" then texto_ord = " DESC"
					
				'sql = "SELECT RQ_SUSTANCIAS.*,RQ_SUSTANCIAS_LPC.campo3 FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_LPC ON RQ_SUSTANCIAS.CAS=RQ_SUSTANCIAS_LPC.CAS "
				'sql = sql & "WHERE RQ_SUSTANCIAS.nombre<>'' AND (nombre LIKE '%"&buscar&"%' OR sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_LPC.cas LIKE '%"&buscar&"%' OR cee LIKE '%"&buscar&"%' OR rd LIKE '%"&buscar&"%') ORDER BY "&ordenacion
				
				sql = "SELECT RQ_SUSTANCIAS.NOMBRE, RQ_SUSTANCIAS_LPC.CAS, RQ_SUSTANCIAS_LPC.SUSTANCIA, RQ_SUSTANCIAS_LPC.ATMOSFERA, RQ_SUSTANCIAS_LPC.AGUA "
				sql = sql & "FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_LPC ON RQ_SUSTANCIAS.CAS = RQ_SUSTANCIAS_LPC.CAS "
				if buscar<>"" then sql = sql & "WHERE (RQ_SUSTANCIAS_LPC.SUSTANCIA LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_LPC.CAS LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.nombre LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.cas LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.cee LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.rd LIKE '%"&buscar&"%') "
				...
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
				  ...
				  <tr>
				  	<td class="subtitulo3">
				  		<a href="risctox_lpc.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_LPC.SUSTANCIA<%=texto_ord%>">Sustancias (<%=registros%>)</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_LPC.SUSTANCIA DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_LPC.SUSTANCIA" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3">
				  		<a href="risctox_lpc.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS.cas<%=texto_ord%>">CAS</a>&nbsp;
				  	...
				  	<td class="subtitulo3" align="center">
				  		<a href="risctox_lpc.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_LPC.ATMOSFERA<%=texto_ord%>">Contaminante de la atmósfera</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS.ATMOSFERA DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS.ATMOSFERA" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3" align="center">
				  		<a href="risctox_lpc.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_LPC.AGUA<%=texto_ord%>">Contaminante del agua</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS.AGUA DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS.AGUA" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  </tr>
<%				objRecordset.movefirst
				...
				<tr>
					...
					<td class="celda_risctox" align="center"><% if objRecordset("atmosfera")="X" then response.write "SÍ" else response.write "NO"%>&nbsp;</td>
					<td class="celda_risctox" align="center"><% if objRecordset("agua")="X" then response.write "SÍ" else response.write "NO"%>&nbsp;</td>
				</tr>
<%				objRecordset.movenext
				reg = reg+1
				loop
...
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="risctox_lpc.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="risctox_lpc.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
<%				end if %>				
				...
				<input type="hidden" name="campo3" value="atmosfera">
				<input type="hidden" name="campo4" value="agua">
				<input type="hidden" name="nombre1" value="nombre/sustancia">
				<input type="hidden" name="nombre2" value="CAS">
				<input type="hidden" name="nombre3" value="contaminante de la atmósfera">
				<input type="hidden" name="nombre4" value="contaminante del agua">
				<input type="hidden" name="sql" value="<%=sql%>">
				<input type="hidden" name="buscar" value="<%=buscar%>">
				<input type="hidden" name="titulo" value="Sustancias (LPCIC)">
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='risctox_lpc.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
</script>
