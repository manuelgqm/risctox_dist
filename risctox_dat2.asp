<% response.redirect "dn_risctox_dat2.asp" %>
<%
...
%>
			...
			<div id="texto">
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "<a href=risctox1.asp>Inicio</a>&nbsp;&gt;&nbsp;Sustancias que causan cambio climático</p>"
              				   else
              				     response.write "<p class=campo>&nbsp;</p>"
              			   end if %>
				
				<p class=titulo3>RISCTOX: Sustancias que causan cambio climático</p>

				<form name="form_buscar" action="risctox_dat2.asp">
				<table class="tabla3" width="90%" align="center">
				  ...
				  <tr>
					<td class="texto">(Cambio climático)&nbsp;</td>
...
<%				buscar = request("buscar")
				...
				if request("ordenacion")="" then 
					ordenacion = "RQ_SUSTANCIAS_DAT2.nombre"
				else
					ordenacion = request("ordenacion")
				end if
				if right(ordenacion,4)<>"DESC" then texto_ord = " DESC"
					
				'sql = "SELECT RQ_SUSTANCIAS.*,RQ_SUSTANCIAS_DAT2.campo3 FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_DAT2 ON RQ_SUSTANCIAS.CAS=RQ_SUSTANCIAS_DAT2.CAS "
				'sql = sql & "WHERE RQ_SUSTANCIAS.nombre<>'' AND (nombre LIKE '%"&buscar&"%' OR sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_DAT2.cas LIKE '%"&buscar&"%' OR cee LIKE '%"&buscar&"%' OR rd LIKE '%"&buscar&"%') ORDER BY "&ordenacion
				
				sql = "SELECT RQ_SUSTANCIAS.NOMBRE, RQ_SUSTANCIAS_DAT2.CAS, RQ_SUSTANCIAS_DAT2.NOMBRE AS SUSTANCIA "
				sql = sql & "FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_DAT2 ON RQ_SUSTANCIAS.CAS = RQ_SUSTANCIAS_DAT2.CAS "
				...
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
				  ...
				  <tr>
				  	<td class="subtitulo3">
				  		<a href="risctox_dat2.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_DAT2.nombre<%=texto_ord%>">Sustancias (<%=registros%>)</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_DAT2.nombre DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_DAT2.nombre" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3">
				  		<a href="risctox_dat2.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_DAT2.cas<%=texto_ord%>">CAS</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_DAT2.cas DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_DAT2.cas" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  </tr>
...
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="risctox_dat2.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="risctox_dat2.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
<%				end if %>				
				...
				<input type="hidden" name="titulo" value="Sustancias que causan cambio climático">
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='risctox_dat2.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
</script>
