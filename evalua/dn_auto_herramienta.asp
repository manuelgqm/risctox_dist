...
sql="select count(*) AS num_productos FROM dn_auto_productos WHERE id_ecogente="&session("id_ecogente2")
...
<%
	...
	'response.write "<!--id_ecogente="&usuario&"-->"
...
<link rel="stylesheet" type="text/css" media="print" href="../dn_estilos_impresion.css">
...
<% end if %>
	...
	<input type="hidden" id="num_componentes" name="num_componentes" value="0">
		...
		<table border="0">
			...
			<tr>
				<td>
					<input type="text" class="campo" size="<%=len_auto_prod_nombre%>" maxlength="750" id="prod_nombre" name="prod_nombre" />
				</td>
				<td><input type="text" class="campo" size="25" maxlength="50" id="prod_frases_r" name="prod_frases_r" /></td>
			</tr>
			<tr>
		...
		<table border="0" width="100%">
			...
			<tr>
				<td>
					<select name="prod_cod_estado">
						<% dameOpciones "cod", "nombre", "dn_auto_estados", "cod", "", "", "", "" %>
					</select>
				</td>
				<td>
					<select name="prod_cod_presion">
			...
			<tr>
				<td>
					<select name="prod_cod_temperatura">
						<% dameOpciones "cod", "nombre", "dn_auto_temperaturas", "cod", "", "", "", "" %>
					</select>
				</td>
				<td>
					<select name="prod_cod_inflamabilidad">
		...
		%>
				...
				<table border="0" width="100%">
					...
					<tr>
						<td>
							<select name="comp<%=contador%>_numero_tipo" id="comp<%=contador%>_numero_tipo" onFocus="busca_sustancia_live('comp<%=contador%>')" onBlur="busca_sustancia_live_stop()">
						...
						<td><input type="text" class="campo" size="25" maxlength="50" id="comp<%=contador%>_numero" name="comp<%=contador%>_numero" onFocus="busca_sustancia_live('comp<%=contador%>')" onBlur="busca_sustancia_live_stop()" /> <!--<input type="button" class="boton2" id="comp<%=contador%>_buscar" name="comp<%=contador%>_buscar" onclick="busca_sustancia('comp<%=contador%>')" value="Buscar" />--></td>
						<td align="center"><input type="text" class="campo" size="4" maxlength="10" name="comp<%=contador%>_porcentaje" /> %</td>
					</tr>
				</table>
				...
				<table border="0" width="100%">
					...
					<tr>
						<td>
							<input type="text" class="campo" size="50" maxlength="750" id="comp<%=contador%>_nombre" name="comp<%=contador%>_nombre" onFocus="busca_sustancia_live('comp<%=contador%>')" onBlur="busca_sustancia_live_stop()" />
						</td>
						<td>
							<input type="text" class="campo" size="25" maxlength="50" id="comp<%=contador%>_frases_r" name="comp<%=contador%>_frases_r" />
						</td>				
					</tr>
				</table>
...
anadir_componente(<%=maxComponentes%>);
	...
	<p align="center">Esta página ha sido desarrollada por <strong><a href="http://www.istas.ccoo.es/">ISTAS</a></strong> que es una Fundación de <strong><a href="http://www.ccoo.es/">CC.OO.</a></strong><br/><br/></p>
			...
			<div id='nuevo_pie' style="text-align:center">
      		</div>
</div>
