<% response.redirect "dn_risctox_enf.asp" %>
<%
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "<a href=risctox1.asp>Inicio</a>&nbsp;&gt;&nbsp;Enfermedades profesionales (borrador)</p>"
              				   else
              				     response.write "<p class=campo>&nbsp;</p>"
              			   end if %>
				
				<p class=titulo3>RISCTOX: Enfermedades profesionales (borrador)</p>

				<form name="form_buscar" action="risctox_enf.asp">
				<table class="tabla3" width="90%" align="center">
				  <tr>
					<td class="subtitulo3" align="right">Buscador de sustancias asociadas a enfermedades profesionales:&nbsp;</td>
				  ...
				  <tr>
					<td class="texto">(borrador)&nbsp;</td>
					<td class="texto">(puedes escribir una parte del nombre o sinónimo, o el CAS)</td>
				  </tr>
				</table>
				</form>
...
<%				buscar = request("buscar")
				...
				sql = "SELECT DISTINCT RQ_SUSTANCIAS.nombre,RQ_SUSTANCIAS.cas,RQ_ENF_PROF.d1 as sinonimo FROM RQ_SUSTANCIAS LEFT JOIN RQ_SUST_ENF ON RQ_SUSTANCIAS.id= RQ_SUST_ENF.sustancia LEFT JOIN RQ_ENF_PROF ON RQ_SUST_ENF.enf_prof=RQ_ENF_PROF.idenfprof "
				sql = sql & "WHERE (RQ_SUST_ENF.cas<>'' AND RQ_SUST_ENF.cas<>'—' AND (RQ_ENF_PROF.d1 LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.nombre LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.cas LIKE '%"&buscar&"%')) ORDER BY "&ordenacion
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
				  ...
				  <tr>
				  	<td class="subtitulo3">
				  		<a href="risctox_enf.asp?buscar=<%=buscar%>&ordenacion=nombre<%=texto_ord%>">Sustancias (<%=registros%>)</a>&nbsp;
				  	...
				  	<td class="subtitulo3">
				  		<a href="risctox_enf.asp?buscar=<%=buscar%>&ordenacion=sinonimo<%=texto_ord%>">Grupo</a>&nbsp;
				  		<% if ordenacion="sinonimo DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="sinonimo" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3">
				  		<a href="risctox_enf.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS.cas<%=texto_ord%>">CAS</a>&nbsp;
...
<%				objRecordset.movefirst
				...
				<tr>
					<td class="celda_risctox"><a href="risctox3.asp?buscar=<%=buscar%>&cas=<%=objRecordset("cas")%>&nombre=<%=objRecordset("nombre")%>" title="<%=objRecordset("nombre")%>"><%=formato(objRecordset("nombre"),30)%></a>&nbsp;</td>
					<td class="celda_risctox" nowrap><%=formato(objRecordset("sinonimo"),30)%>&nbsp;</td>
					<td class="celda_risctox" nowrap><%=formato(objRecordset("cas"),15)%>&nbsp;</td>
				</tr>
<%				objRecordset.movenext
				i = i+1
				loop
...
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="risctox_enf.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="risctox_enf.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
<%				end if %>				
				...
				<input type="hidden" name="campo2" value="sinonimo">
				<input type="hidden" name="campo3" value="cas">
				<input type="hidden" name="nombre1" value="NOMBRE">
				<input type="hidden" name="nombre2" value="GRUPO">
				<input type="hidden" name="nombre3" value="CAS">
				<input type="hidden" name="sql" value="<%=sql%>">
				<input type="hidden" name="buscar" value="<%=buscar%>">
				<input type="hidden" name="titulo" value="Listado de sustancias asociadas a enfermedades profesionales">
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='risctox_enf.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
</script>