<%
	...
	case 1: 'han dado a buscar
		...
		'seg�n filtro, unimos a distintas tablas, nos indica de que buscador venimos
		if filtro<>"0" then
			...
			select case filtro
				...
				case "1":
					...
					' y tambi�n sustancias asociadas a ficheros de alternativas
...
' ***** FIN NUEVAS LISTAS SPL
				...
				case "sen": ' sensibilizante... no hace falta m�s tablas que la principal
				...
				case "pyb": 'Sustancias t�xicas, persistentes y bioacumulativas
				sqls=sqls & " LEFT OUTER JOIN dn_risc_sustancias_ambiente ON (sus.id=dn_risc_sustancias_ambiente.id_sustancia)"

				case "tac": 'Sustancias de toxicidad acu�tica seg�n Directiva de aguas
				sqls=sqls & " LEFT OUTER JOIN dn_risc_sustancias_ambiente ON (sus.id=dn_risc_sustancias_ambiente.id_sustancia)"

				case "tac2": 'Sustancias peligrosas agua Alemania
				sqls=sqls & " LEFT OUTER JOIN dn_risc_sustancias_ambiente ON (sus.id=dn_risc_sustancias_ambiente.id_sustancia)"

				case "dat": 'Sustancias de da�o a la capa de ozono
				sqls=sqls & " LEFT OUTER JOIN dn_risc_sustancias_ambiente ON (sus.id=dn_risc_sustancias_ambiente.id_sustancia)"

				case "dat2": 'Sustancias cambio clim�tico
				sqls=sqls & " LEFT OUTER JOIN dn_risc_sustancias_ambiente ON (sus.id=dn_risc_sustancias_ambiente.id_sustancia)"

				case "dat3": 'Sustancias calidad aire
				sqls=sqls & " LEFT OUTER JOIN dn_risc_sustancias_ambiente ON (sus.id=dn_risc_sustancias_ambiente.id_sustancia)"

				case "vl1": 'L�mites de exposici�n profesional: Valores L�mite Ambientales
				sqls=sqls & " LEFT OUTER JOIN dn_risc_sustancias_vl ON (sus.id=dn_risc_sustancias_vl.id_sustancia)"

				case "vl2": '	Valores L�mite Ambientales Cancer�genos
				sqls=sqls & " LEFT OUTER JOIN dn_risc_sustancias_vl as vl ON (sus.id=vl.id_sustancia)"

				case "vl3": 'L�mites de exposici�n profesional: Valores L�mite Biol�gicos
				sqls=sqls & " LEFT OUTER JOIN dn_risc_sustancias_vl ON (sus.id=dn_risc_sustancias_vl.id_sustancia)"
				...
				case "cov": 'Compuestos org�nicos vol�tiles (COV)
				sqls=sqls & " LEFT OUTER JOIN dn_risc_sustancias_ambiente ON (sus.id=dn_risc_sustancias_ambiente.id_sustancia)"
		...
		'seg�n filtro, agregamos distintas condiciones
		if filtro<>"0" then
          ...
          frases="R40, R45, R49, R40/20, R40/21, R40/22, R40/20/21, R40/20/22, R40/21/22, R40/20/21/22, R46, R68, R68/20, R68/21, R68/22, R68/20/21, R68/20/22, R68/21/22, R68/20/21/22"
				...
				case "cym2": 'la condicion simplemente es que exista la fila, pero ya que estamos, comprobamos que no este vac�a
				  ...
				  'Sergio
				  'sqls=sqls & "( ((dn_risc_sustancias_iarc.grupo_iarc<>'') OR ("&monta_condicion_grupo("asoc_cancer_iarc")&")) and dn_risc_sustancias_iarc.grupo_iarc<>'GRUPO 4')"



				case "cym3": 'la condicion simplemente es que exista la fila, pero ya que estamos, comprobamos que no este vac�a
...
' ***** FIN NUEVAS LISTAS SPL
          ...
          sqls = sqls & "( " & sql_lista_neurotoxico & " OR ("&monta_condicion_grupo("asoc_neuro_oto")&") )"
		  		...
		  		case "oto":
          		'Comentar con Xavi
				'sqls = sqls & " dn_risc_sustancias_neuro_disruptor.efecto_neurotoxico='OTOTÓXICO' and ( " & sql_lista_neurotoxico & " OR ("&monta_condicion_grupo("asoc_neuro_oto")&") )"
				sqls = sqls & " dn_risc_sustancias_neuro_disruptor.efecto_neurotoxico='OTOT�XICO'" ' and ( " & sql_lista_neurotoxico & " OR ("&monta_condicion_grupo("asoc_neuro_oto")&") )"
				...
				case "enf": 'la condicion simplemente es que exista la fila, pero ya que estamos, comprobamos que no este vac�a
				...
				case "res": 'la condicion es que exista la fila O QUE que num_rd <>'' / como tenemos full outer, a�adimos la condicion de que la tabla de sustancias no este vacia, por si acaso
...
'response.write "<br>pag: "&pag
'response.write "<br>arr: "&arr
