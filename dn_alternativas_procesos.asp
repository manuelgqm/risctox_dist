...
<td><p class=campo>Est&aacute;s en: <a href=index.asp?idpagina=550>Plataforma prevención de riesgo químico</a>&nbsp;&gt;&nbsp;<a href="dn_alternativas_portada.asp">BBDD Alternativas</a> &gt; Procesos </p></td>
...
sqll="SELECT DISTINCT LEFT(nombre, 1) AS letter FROM dn_alter_procesos order by letter"
...
<%
	sqlmiletra="select id, nombre from dn_alter_procesos where nombre like '" &letra& "%' order by nombre"
	...
	if rstl.eof then
		...
		for contadorFilas=0 to ubound(arrayDatos,2)
			tablares=tablares& "<tr><td class='celda_risctox'><a href='dn_alternativas_ficha_proceso.asp?id=" &arrayDatos(0,contadorFilas)& "'>" &arrayDatos(1,contadorFilas)& "</a></td><tr>"
		next
...
function hayresultados(letra)
	...
	if instr(lista,letra) then letra = "<a href='dn_alternativas_procesos.asp?letra="&letra&"'>" &letra& "</a>"
