				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "<a href=risctox1.asp>Inicio</a>&nbsp;&gt;&nbsp;Sustancias sensibilizantes</p>"
              				   else
              				     response.write "<p class=campo>&nbsp;</p>"
              			   end if %>
				
				<p class=titulo3>RISCTOX: Sustancias sensibilizantes</p>

				<form name="form_buscar" action="risctox_sen.asp">
				<table class="tabla3" width="90%" align="center">
				  <tr>
					<td class="subtitulo3" nowrap align="right">Buscador de sustancias sensibilizantes:&nbsp;</td>
...
<%				buscar = request("buscar")
				...
				sql = "SELECT nombre,cas,cee,clasific1,clasific2 FROM RQ_SUSTANCIAS WHERE ((RD<>'' OR isnull(RD,'nulo')<>'nulo') AND (1=0"
				for i=1 to 11
					sql = sql &" OR clasific"&i&" LIKE '%R68%' " 
				next
				...
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
				  ...
				  <tr>
				  	<td class="subtitulo3">
				  		<a href="risctox_sen.asp?buscar=<%=buscar%>&ordenacion=nombre<%=texto_ord%>">Sustancias (<%=registros%>)</a>&nbsp;
				  	...
				  	<td class="subtitulo3">
				  		<a href="risctox_sen.asp?buscar=<%=buscar%>&ordenacion=cas<%=texto_ord%>">CAS</a>&nbsp;
				  	...
				  	<td class="subtitulo3">
				  		<a href="risctox_sen.asp?buscar=<%=buscar%>&ordenacion=cee<%=texto_ord%>">Nº CE</a>&nbsp;
				  	...
				  	<td class="subtitulo3">
				  		<a href="risctox_sen.asp?buscar=<%=buscar%>&ordenacion=rd<%=texto_ord%>">CLASIF1</a>&nbsp;
				  	...
				  	<td class="subtitulo3">
				  		<a href="risctox_sen.asp?buscar=<%=buscar%>&ordenacion=rd<%=texto_ord%>">CLASIF2</a>&nbsp;
				  		<% if ordenacion="rd DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="rd" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  </tr>
<%				objRecordset.movefirst
				...
				<tr>
					<td class="celda_risctox"><a href="risctox3.asp?buscar=<%=buscar%>&cas=<%=objRecordset("cas")%>" title="<%=objRecordset("nombre")%>"><%=formato(objRecordset("nombre"),30)%></a>&nbsp;</td>
					<td class="celda_risctox" nowrap><%=formato(objRecordset("cas"),15)%>&nbsp;</td>
					<td class="celda_risctox" nowrap><%=formato(objRecordset("cee"),15)%>&nbsp;</td>
					<td class="celda_risctox" nowrap><%=formato(objRecordset("clasific1"),50)%>&nbsp;</td>
					<td class="celda_risctox" nowrap><%=formato(objRecordset("clasific2"),50)%>&nbsp;</td>
				</tr>
<%				objRecordset.movenext
				i = i+1
				loop
...
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="risctox_sen.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="risctox_sen.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
<%				end if %>				
				...
				<input type="hidden" name="campo4" value="clasific1">
				<input type="hidden" name="campo5" value="clasific2">
				...
				<input type="hidden" name="nombre4" value="clasific1">
				<input type="hidden" name="nombre5" value="clasific2">
				<input type="hidden" name="sql" value="<%=sql%>">
				<input type="hidden" name="buscar" value="<%=buscar%>">
				<input type="hidden" name="titulo" value="Listado de sustancias sensibilizantes">
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='risctox_sen.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
</script>