<% response.redirect "dn_risctox_cym3.asp" %>
<%
...
%>
			...
			<div id="texto">
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "<a href=risctox1.asp>Inicio</a>&nbsp;&gt;&nbsp;Sustancias cancerígenas o mutágenas (según otras fuentes)</p>"
              				   else
              				     response.write "<p class=campo>&nbsp;</p>"
              			   end if %>
				
				<p class=titulo3>RISCTOX: Sustancias cancerígenas o mutágenas (según otras fuentes)</p>

				<form name="form_buscar" action="risctox_cym3.asp">
				<table class="tabla3" width="90%" align="center">
				  ...
				  <tr>
					<td class="texto">(según otras fuentes)&nbsp;</td>
...
<%				buscar = request("buscar")
				...
				if request("ordenacion")="" then 
					ordenacion = "RQ_SUSTANCIAS_CYM3.nombre"
				else
					ordenacion = request("ordenacion")
				end if
				if right(ordenacion,4)<>"DESC" then texto_ord = " DESC"
					
				'sql = "SELECT RQ_SUSTANCIAS.*,RQ_SUSTANCIAS_CYM3.fuente FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_CYM3 ON RQ_SUSTANCIAS.CAS=RQ_SUSTANCIAS_CYM3.CAS "
				'sql = sql & "WHERE RQ_SUSTANCIAS.nombre<>'' AND (nombre LIKE '%"&buscar&"%' OR sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_CYM3.cas LIKE '%"&buscar&"%' OR cee LIKE '%"&buscar&"%' OR rd LIKE '%"&buscar&"%') ORDER BY "&ordenacion
				
				sql = "SELECT RQ_SUSTANCIAS.NOMBRE, RQ_SUSTANCIAS_CYM3.CAS, RQ_SUSTANCIAS_CYM3.nombre as SUSTANCIA, RQ_SUSTANCIAS_CYM3.fuente FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_CYM3 ON RQ_SUSTANCIAS.CAS=RQ_SUSTANCIAS_CYM3.CAS "
				if buscar<>"" then sql = sql & "WHERE (RQ_SUSTANCIAS_CYM3.CAS LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_CYM3.nombre LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.nombre LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.cas LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.cee LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.rd LIKE '%"&buscar&"%') "
				...
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
				  ...
				  <tr>
				  	<td class="subtitulo3">
				  		<a href="risctox_cym3.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_CYM3.nombre<%=texto_ord%>">Sustancias (<%=registros%>)</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM3.nombre DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM3.nombre" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3">
				  		<a href="risctox_cym3.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_CYM3.cas<%=texto_ord%>">CAS</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM3.cas DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM3.cas" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3">
				  		<a href="risctox_cym3.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_CYM3.fuente<%=texto_ord%>">FUENTE</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM3.fuente DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM3.fuente" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  </tr>
<%				objRecordset.movefirst
				...
				<tr>
					...
					<% if not isnull(objRecordset("fuente")) then
...
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="risctox_cym3.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="risctox_cym3.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
<%				end if %>				
				...
				<input type="hidden" name="campo1" value="RQ_SUSTANCIAS_CYM3.nombre">
				<input type="hidden" name="campo2" value="RQ_SUSTANCIAS_CYM3.cas">
				...
				<input type="hidden" name="titulo" value="Listado de sustancias cancerígenas o mutágenas (según otras fuentes)">
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='risctox_cym3.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
</script>
