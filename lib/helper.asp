...
function monta_frases_rh_epa(byval frases_rh, tipo)
	' Las frases R epas vienen separadas por espacios, y para cada una si tiene simbolo, separado por punto y coma
	...
	' Devolvemos las frases R epas
	monta_frases_rh_epa = frases
end function
...
function esta_en_lista(byval lista, byval id_sustancia)
	...
	campos_epa = "sus.frases_r_danesa"
	select_campos = "select distinct sus.id, sus.nombre from dn_risc_sustancias as sus "
	join_epa = "inner join ist_risc_sustancias_epa as epa on sus.id = epa.id_sustancia "

	sql_lista_cancer_rd = select_campos & "WHERE " & monta_condicion(campos_rd, frases_r_cancer)
	sql_lista_cancer_rd_h = select_campos & "WHERE " & monta_condicion(campos_rd, frases_h_cancer)
	sql_lista_cancer_epa = select_campos & "WHERE " & monta_condicion(campos_epa, frases_r_cancer)
	sql_lista_cancer_epa_h = select_campos & join_epa & "WHERE " & monta_condicion(campos_epa, frases_h_cancer)
	sql_lista_mutageno_rd = select_campos & "WHERE " & monta_condicion(campos_rd, frases_r_mutageno)
	sql_lista_mutageno_rd_h = select_campos & "WHERE " & monta_condicion(campos_rd, frases_h_mutageno)
	sql_lista_mutageno_epa = select_campos & "WHERE " & monta_condicion(campos_epa, frases_r_mutageno)
	sql_lista_mutageno_epa_h = select_campos & join_epa & "WHERE " & monta_condicion(campos_epa, frases_h_mutageno)
	...
	' TPR
	sql_lista_tpr = select_campos & "WHERE " & monta_condicion(campos_rd, frases_r_tpr)
	sql_lista_tpr_epa = select_campos & "WHERE " & monta_condicion(campos_epa, frases_r_tpr)
	sql_lista_tpr_epa_h = select_campos & join_epa & "WHERE " & monta_condicion(campos_epa, frases_h_tpr)
	...
	sql_lista_neurotoxico_rd = select_campos & "WHERE " & monta_condicion(campos_rd, frases_r_neurotox)
	sql_lista_neurotoxico_rd_h = select_campos & "WHERE " & monta_condicion(campos_rd, frases_h_neurotox)
	sql_lista_neurotoxico_epa = select_campos & "WHERE " & monta_condicion(campos_epa, frases_r_neurotox)
	sql_lista_neurotoxico_epa_h = select_campos & join_epa & "WHERE " & monta_condicion(campos_epa, frases_h_neurotox)

	' NEUROTOXICO PARA LISTADO (SIMILAR A FUNCIONAMIENTO DE LISTA NEGRA, SOLO CONDICION)
	sql_lista_neurotoxico = "(sus.id IN ("&qn(sql_lista_neurotoxico_nivel)&") OR sus.id IN ("&qn(sql_lista_neurotoxico_rd)&") OR sus.id IN ("&qn(sql_lista_neurotoxico_epa) & "OR sus.id IN ("&qn(sql_lista_neurotoxico_epa_h) & "))"

	' SENSIBILIZANTE
	sql_lista_sensibilizante = select_campos & "WHERE " & monta_condicion(campos_rd, frases_r_sensib)
	sql_lista_sensibilizante_h = select_campos & "WHERE " & monta_condicion(campos_rd, frases_h_sensib)
	sql_lista_sensibilizante_epa = select_campos & "WHERE " & monta_condicion(campos_epa, frases_r_sensib)
	sql_lista_sensibilizante_epa_h = select_campos & join_epa & "WHERE " & monta_condicion(campos_epa, frases_h_sensib)
	...
	select case lista
		...
		case "cancer_epa": ' Cancerigeno según lista epa
			sql_lista = sql_lista_cancer_epa
			
		case "cancer_epa_h": 'Cancerigeno según frases H lista epa
			sql_lista = sql_lista_cancer_epa_h
      ...
      sql_lista = sql_lista_mutageno_rd_h
		...
		case "mutageno_epa": ' Mutágeno según lista epa
      sql_lista = sql_lista_mutageno_epa
			
		case "mutageno_epa_h": ' Mutágeno según lista epa frases H
      sql_lista = sql_lista_mutageno_epa_h
      ...
      sql_lista = parentesis_where(sql_lista_tpr_h) & " OR ("&monta_condicion_grupo("asoc_reproduccion")&") )"
		...
		case "tpr_epa": ' Tóxicos para la reproducción según lista epa
      sql_lista = sql_lista_tpr_epa
			
		case "tpr_epa_h": ' Tóxicos para la reproducción según lista epa
      sql_lista = sql_lista_tpr_epa_h

		case "de": ' Disruptor endocrino
      sql_lista = parentesis_where(sql_lista_de) & " OR ("&monta_condicion_grupo("asoc_disruptores")&") )"

		case "neurotoxico": ' Neurótoxico (RD o epa o por nivel)
      ...
      sql_lista = sql_lista_neurotoxico_rd_h
		...
		case "neurotoxico_epa": ' Neurótoxico epa
      sql_lista = sql_lista_neurotoxico_epa
			
		case "neurotoxico_epa_h": ' Neurótoxico epa
      sql_lista = sql_lista_neurotoxico_epa_h

		case "neurotoxico_nivel": ' Neurótoxico epa
      ...
      sql_lista = sql_lista_sensibilizante_h
		...
		case "sensibilizante_epa": ' Sensibilizante según lista epa
      sql_lista = sql_lista_sensibilizante_epa
		
		case "sensibilizante_epa_h": ' Sensibilizante según lista epa
      sql_lista = sql_lista_sensibilizante_epa_h
...
'response.write sql_lista
	...
	response.write "<br /><br />/*" & lista & "*/<br/>" & sql_lista & ";"
...
function monta_condicion(byval campos, byval frases)
  ...
  for c = 0 to ubound(array_campos)
    ...
    for f = 0 to ubound(array_frases)
      ...
      ' Buscamos al inicio del campo, o separado por ; o separado por espacio (lista epa)
...
function getSubstanceAttributes( sustancia_id )
	dim sqlSt, rs, output
	...
	rs.Open sqlSt, objConnection2, adOpenKeyset
	response.write(rs.Fields.Count)
	response.end
	set output = CreateObject( "Scripting.Dictionary" )
	output.add "sustancia_id", rs("sustancia_id").value
	output.add "listas", rs("listas").value	
	output.add "categorias", rs("categorias").value
	Set getSubstanceAttributes = output
