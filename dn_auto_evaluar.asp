...
if (ids_evaluar = "") then
	...
	sql = "SELECT nombre, id FROM dn_auto_productos WHERE ((id IN ("&ids_evaluar&")) AND (id_ecogente="&session("id_ecogente")&"))"
...
function pintatox(riesgo,razon)
			...
			if riesgo=0 then
				cadtox=cadtox& "<a href=" &chr(34)& "javascript:mostrar_ayuda('INFORMACION INSUFICIENTE')" &chr(34)& ">Información insuficiente</a>"
			else			
