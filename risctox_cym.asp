<% response.redirect "dn_risctox_cym.asp" %>
<%
...
%>
			...
			<div id="texto">
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "<a href=risctox1.asp>Inicio</a>&nbsp;&gt;&nbsp;Sustancias cancerígenas o mutágenas (según RD 363/1995)</p>"
              				   else
              				     response.write "<p class=campo>&nbsp;</p>"
              			   end if %>
				
				<p class=titulo3>RISCTOX: Sustancias cancerígenas o mutágenas (según RD 363/1995)</p>

				<form name="form_buscar" action="risctox_cym.asp">
				<table class="tabla3" width="90%" align="center">
				  ...
				  <tr>
					<td class="texto">(según RD 363/1995)&nbsp;</td>
...
<%				buscar = request("buscar")
				...
				if request("ordenacion")="" then 
					ordenacion = "RQ_SUSTANCIAS_CYM.SUSTANCIA"
				else
					ordenacion = request("ordenacion")
				end if
				if right(ordenacion,4)<>"DESC" then texto_ord = " DESC"
					
				'sql = "SELECT RQ_SUSTANCIAS.*,RQ_SUSTANCIAS_CYM.C,RQ_SUSTANCIAS_CYM.M FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_CYM ON RQ_SUSTANCIAS.CAS=RQ_SUSTANCIAS_CYM.CAS "
				'sql = sql & "WHERE RQ_SUSTANCIAS.nombre<>'' AND (nombre LIKE '%"&buscar&"%' OR sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_CYM.cas LIKE '%"&buscar&"%' OR cee LIKE '%"&buscar&"%' OR rd LIKE '%"&buscar&"%') ORDER BY "&ordenacion

				sql = "SELECT RQ_SUSTANCIAS.NOMBRE, RQ_SUSTANCIAS_CYM.CAS, RQ_SUSTANCIAS_CYM.SUSTANCIA, RQ_SUSTANCIAS_CYM.C, RQ_SUSTANCIAS_CYM.M FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_CYM ON RQ_SUSTANCIAS.CAS=RQ_SUSTANCIAS_CYM.CAS "
				sql = sql &" WHERE (RQ_SUSTANCIAS_CYM.CAS LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_CYM.SUSTANCIA LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.nombre LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.cas LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.cee LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.rd LIKE '%"&buscar&"%') "
				...
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
				  ...
				  <tr>
				  	<td class="subtitulo3">
				  		<a href="risctox_cym.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_CYM.SUSTANCIA<%=texto_ord%>">Sustancias (<%=registros%>)</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM.SUSTANCIA DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM.SUSTANCIA" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3">
				  		<a href="risctox_cym.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_CYM.cas<%=texto_ord%>">CAS</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM.cas DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM.cas" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3">
				  		<a href="risctox_cym.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_CYM.c<%=texto_ord%>">Cancerígeno</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM.c DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM.c" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3">
				  		<a href="risctox_cym.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_CYM.m<%=texto_ord%>">Mutágeno</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM.m DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM.m" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  </tr>
...
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="risctox_cym.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="risctox_cym.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
<%				end if %>				
				...
				<input type="hidden" name="campo3" value="c">
				<input type="hidden" name="campo4" value="m">
				<input type="hidden" name="nombre1" value="nombre/sustancia">
				<input type="hidden" name="nombre2" value="cas">
				<input type="hidden" name="nombre3" value="Cancerígeno">
				<input type="hidden" name="nombre4" value="Mutágeno">
				<input type="hidden" name="sql" value="<%=sql%>">
				<input type="hidden" name="buscar" value="<%=buscar%>">
				<input type="hidden" name="titulo" value="Listado de sustancias cancerígenas o mutágenas (según RD 363/1995)">
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='risctox_cym.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
