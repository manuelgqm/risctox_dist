...
<%
	...
	if session("id_ecogente")="" then response.redirect "acceso.asp?idpagina=571"
	

	texto_busca = limpia(cstr(request("texto_busca")))
	Tipo = request("Tipo")
	If Tipo = "" Then Tipo = "0"
	idSubtipo_Ambiental = request("subTipo")
	If idSubtipo_Ambiental = "" Then idSubtipo_Ambiental = "0"
	ambito = request("ambito")
	If ambito = "" Then ambito = "0"
	idautonomia = request("idautonomia")
	If idautonomia = "" Then idautonomia = "0"
	If CStr(ambito) = "2" Then idSubtipo_Ambiental = "0"
...
<head>
<title>ECOinformas: Búsqueda en vigilancia legislativa</title>
...
%>
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write titulo&"<a href='vig_leg_busca.asp'>Inicio</a></p>"
              				   else
			...
			If sqlWhere <> "" then
				 %>
					<p class="titulo2">Resultados de la búsqueda en la base de datos de vigilancia legislativa:</p>
					<p class="texto"><b>Texto:</b>&nbsp;<%=request("texto_busca")%><br><b>Ámbito territorial:</b>&nbsp;<%=texto_ambito_te %><br><b>Aspectos ambientales:</b>&nbsp;<%=texto_ambito_am %>
					</p>
			<% Else %>
					<p class="texto">Introduce una o varias palabras claves para buscar en el título o texto de la ley, también puedes buscar por criterios ambientales o territoriales. Si el número de resultados es muy grande, puedes refinar la búsqueda añadiendo más parámetros y consultando de nuevo.
					</p>
			...
			If sqlWhere <> "" then %>
					<p class="texto">
					<% if num_resultados=0 And (CStr(idautonomia) <> "" And CStr(idautonomia) <> "0") then %>
					No hay ninguna ley con estos parámetros. <A HREF="vig_leg_busca.asp?Tipo=<%=Tipo%>&ambito=1&idautonomia=&subtipo=&texto_busca=<%=texto_busca%>">Ver legislación estatal de aplicación.</A>
					<% elseif num_resultados=0 And (CStr(idautonomia) = "" or CStr(idautonomia) = "0") then %>
					No hay ninguna ley con estos parámetros. Vuelve a buscar con otros parámetros.
					<% else %>
					Hay <%=num_resultados%> leyes. Pulsa sobre una ley para ver sus detalles.
					<% end if %>
					</p>
					...
					<tr><td class="texto" valign="top" align="right" width="10%"><a href="vig_leg3.asp?id=<%=objRecordset("idLey")%>"><img src="imagenes/ico_puntito.gif" valign="top" border=0></a></td><td class="texto" align="left" bgcolor="#<%=color%>"><a href="vig_leg3.asp?id=<%=objRecordset("idLey")%>"><%=objRecordset("titulo")%></td></tr>
					<% i=-1*i
					   objRecordset.movenext
					   loop %>
					</table>
