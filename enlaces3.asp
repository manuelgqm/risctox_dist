<%
	...
	idpagina = 674	'--- página listado enlaces  por sectores (sólo para registrar estadísticas)
	'----- Registrar la visita
	...
	sql = "SELECT numeracion FROM ENL_TEMAS WHERE id=329"	'-- Actividades de formación
	set objRecordset = Server.CreateObject ("ADODB.Recordset")
	set objRecordset = OBJConnection.Execute(sql)
	numeracion2 = objrecordset("numeracion")

	sql = "SELECT numeracion FROM ENL_TEMAS WHERE id=341"	'-- Servicio de documentación (Vig.Tecnológica y Enlaces web)
	set objRecordset = Server.CreateObject ("ADODB.Recordset")
	set objRecordset = OBJConnection.Execute(sql)
...
%>
			...
			<% end if %>
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "<a href=index.asp?idpagina=572>Enlaces</a>&nbsp;&gt;&nbsp;<a href='enlaces1.asp'>Inicio</a></p>"
              				   else
              				     response.write "<p class=campo>&nbsp;</p>"
              				   end if %>
				

					<% sql = "SELECT DISTINCT ENL_ENLACES.Id,ENL_ENLACES.afiliacion,ENL_ENLACES.titulo,ENL_ENLACES.url FROM ENL_ENLACES LEFT JOIN ENL_CLASIFICACION ON ENL_ENLACES.Id = ENL_CLASIFICACION.enlace LEFT JOIN ENL_TEMAS ON ENL_CLASIFICACION.tema = ENL_TEMAS.id WHERE (ENL_TEMAS.numeracion LIKE '"&numeracion1&"%' OR ENL_TEMAS.numeracion LIKE '"&numeracion2&"%' OR ENL_TEMAS.numeracion LIKE '"&numeracion3&"%' OR ENL_TEMAS.numeracion LIKE '"&numeracion4&"%') AND (ENL_ENLACES.afiliacion LIKE '%"&request("busca_enlaces")&"%' OR ENL_ENLACES.titulo LIKE '%"&request("busca_enlaces")&"%') ORDER BY ENL_ENLACES.id DESC"
			   		   'response.write "<!--"&sql&"-->"
			   		   set objRecordset = Server.CreateObject ("ADODB.Recordset")
			   		   objRecordset.Open sql,objConnection,adOpenKeyset
			   		   num_resultados = objRecordset.recordcount %>

					<p class="titulo2">Buscando:&nbsp;<%=request("busca_enlaces")%></p>
					<p class=texto">Hay <%=num_resultados%> enlaces.<% if num_resultados>0 then %> Pulsa sobre uno de ellos para visitarlo<%else%> Prueba con otra palabra<% end if %></p>
				...
				<% end if %>
					...
					<p class="titulo2">Si quieres eligir otro grupo de enlaces de interés pulsa sobre el que quieres consultar:</p>
					<table align="center" width="95%" cellpadding=0 cellspacing=5 border=0 style="background-color: #EFEFEF;" class="tabla2"><tr><td>
					<table style="background: url(imagenes/buscador.gif); background-repeat: no-repeat; background-position: top left; color: #EFEFEF;"><tr><td>
					<table width="95%" cellpadding=0 cellspacing=5 border=0>
					<% sql = "SELECT numeracion FROM ENL_TEMAS WHERE id=716"
					   set objRecordset = Server.CreateObject ("ADODB.Recordset")
					   set objRecordset = OBJConnection.Execute(sql)
			   		   if not objRecordset.eof then numeracion = objRecordset("numeracion")
					   sql = "SELECT id,nombre FROM ENL_TEMAS WHERE numeracion LIKE '" & numeracion & "%' AND numeracion<>'" & numeracion & "' ORDER BY numeracion"
					   set objRecordset = Server.CreateObject ("ADODB.Recordset")
			   		   set objRecordset = OBJConnection.Execute(sql)
			   		   do while not objRecordset.eof %>
					<tr><td class="texto"><img src="imagenes/flecha.gif" border="0" align="absmiddle">&nbsp;<a href="enlaces2.asp?idtipo=<%=objRecordset("id")%>"><%=ucase(objRecordset("nombre"))%></a></td></tr>
					<% objRecordset.movenext
					   loop %>
					</table>
					</td></tr></table>
					...
					<form name="busca_enlaces" method="POST" action="enlaces3.asp">
					<p class="titulo2">o busca enlaces relacionados con todas las áreas de actividad del proyecto ECOinformas:</p>
						<table align="center" width="95%" cellpadding=0 cellspacing=5 border=0 style="background-color: #EFEFEF;" class="tabla2">	
							<tr><td>
								Escribe una palabra contenida en el título del documento:<br><br><input type="text" size="50" maxlength="50" class="campo" name="busca_enlaces">&nbsp;<input type="submit" class="boton" value="busca enlaces">
							</td></tr>
						</table>
					</p>
					</form>
