				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write titulo&"<a href=index.asp?idpagina=571>Vigilancia legislativa</a>&nbsp;&gt;&nbsp;<a href='vig_leg1.asp'>Inicio</a></p>"
              				   else
              				     response.write "<p class=campo>&nbsp;</p>"
              				   end if %>
				
					<% sql = "SELECT idtipo,nombre FROM ECO_VIG_LEG_TIPOS WHERE idtipo="&request("idtipo")
			   		   set objRecordset = Server.CreateObject ("ADODB.Recordset")
			   		   set objRecordset = OBJConnection.Execute(sql)
			   		   industria = objRecordset("nombre") %>

					<% sql = "SELECT ID,normativa FROM ECO_VIG_LEG_LEYES WHERE industria="&request("idtipo")&" ORDER BY ID"
			   		   set objRecordset = Server.CreateObject ("ADODB.Recordset")
					...
					<p class=texto">Hay <%=num_resultados%> leyes. Pulsa sobre una ley para ver sus detalles</p>
					...
					<tr><td class="texto" valign="top" align="right"><a href="vig_leg3.asp?id=<%=objRecordset("ID")%>"><img src="imagenes/ico_puntito.gif" valign="top" border=0></a></td><td class="texto" bgcolor="#<%=color%>"><a href="vig_leg3.asp?id=<%=objRecordset("ID")%>"><%=objRecordset("normativa")%></td></tr>
					<% i=-1*i
					   objRecordset.movenext
					   loop %>
					</table>
					...
					<form name="buscador" action="vig_leg_busca.asp" method="POST">
					...
					<tr><td class="texto" align="left" colspan=3>Texto incluido en el título de la ley:&nbsp<input type="text" size="50" name="busca_texto" class="campo"></td></tr>
					</table>
					<table width="95%" cellpadding=0 cellspacing=5 border=0>
					<tr><td class="texto" align="left" nowrap>Ámbito territorial:&nbsp;<select name="busca_ambito_te" class="campo">
						<option value="">- cualquiera -</option>
						<option value="1">Estatal</option>
						<option value="2">Autonómico</option>
						</select></td>
					    	<td class="texto" align="right" colspan="2" nowrap>Aspectos ambientales:&nbsp;<select name="busca_ambito_am" class="campo">
						<option value="">- cualquiera -</option>
						<option value="3">Residuos</option>
						<option value="4">Vertidos</option>
						<option value="5">Atmósfera</option>
						<option value="6">Ruidos, vibraciones</option>
						<option value="7">Suelos contaminados</option>
						<option value="8">SGMA</option>
						<option value="9">Sostenibilidad</option>
						<option value="10">Consumo de materias primas</option>
						<option value="11">Uso de químicos</option>
						<option value="12">Sociopolíticas medioambientales</option>
						<option value="13">Permisos, autorizaciones</option>
						<option value="14">Formación ambiental</option>
						</select></td></tr>
					</table>
					<table width="95%" cellpadding=0 cellspacing=5 border=0>
					<tr><td class="texto" align="left" colspan=2 nowrap>Ámbito sectorial:&nbsp;<select name="busca_industria" class="campo">
						<option value="">- cualquiera -</option>
						<% sql = "SELECT idtipo,nombre FROM ECO_VIG_LEG_TIPOS ORDER BY idtipo"
			   		   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
			   		   	set objRecordset = OBJConnection.Execute(sql)
			   		   	do while not objRecordset.eof %>
						<option value="<%=objRecordset("idtipo")%>"><%=objRecordset("nombre")%></option>
						<% objRecordset.movenext
					   	loop %>
						</select></td>
