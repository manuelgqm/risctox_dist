...
<td><p class=campo>Est&aacute;s en: <a href=index.asp?idpagina=550>Plataforma prevención de riesgo químico</a>&nbsp;&gt;&nbsp;<a href="dn_alternativas_portada.asp">BBDD Alternativas</a> &gt; Ficha de proceso </p></td>
...
<%
	...
	set rstl=objConnection2.Execute("select nombre, descripcion from dn_alter_procesos where id =" &id)
...
<%
	set rstl=objConnection2.Execute("select dn_alter_ficheros.id, dn_alter_ficheros.titulo, dn_alter_ficheros.num_alternativa from dn_alter_ficheros_por_procesos JOIN dn_alter_ficheros ON dn_alter_ficheros_por_procesos.id_fichero=dn_alter_ficheros.id where dn_alter_ficheros.tema<>'Casos prácticos' and dn_alter_ficheros_por_procesos.id_proceso =" &id & "ORDER BY dn_alter_ficheros.titulo")
	if rstl.eof then
...
<%
	...
	set rstl=objConnection2.Execute("select dn_alter_ficheros.id, dn_alter_ficheros.titulo from dn_alter_ficheros_por_procesos JOIN dn_alter_ficheros ON dn_alter_ficheros_por_procesos.id_fichero=dn_alter_ficheros.id where dn_alter_ficheros.tema='Casos prácticos' and dn_alter_ficheros_por_procesos.id_proceso =" &id & " AND dn_alter_ficheros.num_alternativa NOT IN ("&nums_alternativas&"-1) ORDER BY dn_alter_ficheros.titulo")
	if rstl.eof then
