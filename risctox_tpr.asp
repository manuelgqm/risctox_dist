<% response.redirect "dn_risctox_tpr.asp" %>
<%
	...
	if session("id_ecogente")="" then response.redirect "acceso.asp?pagina_esp=risctox_tpr.asp"
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "<a href=risctox1.asp>Inicio</a>&nbsp;&gt;&nbsp;Sustancias tóxicas para la reproducción</p>"
              				   else
              				     response.write "<p class=campo>&nbsp;</p>"
              			   end if %>
				
				<p class=titulo3>RISCTOX: Sustancias tóxicas para la reproducción</p>

				<form name="form_buscar" action="risctox_tpr.asp">
				<table class="tabla3" width="90%" align="center">
				  <tr>
					<td class="subtitulo3" nowrap align="right">Buscador de sustancias tóxicas para la reproducción:&nbsp;</td>
...
<%				buscar = request("buscar")
				...
				sql = "SELECT nombre,cas,rd,cee FROM RQ_SUSTANCIAS WHERE ((RD<>'' OR isnull(RD,'nulo')<>'nulo') AND (1=0"
				for i=1 to 11
					sql = sql &" OR clasific"&i&" LIKE '%60%' OR clasific"&i&" LIKE '%61%' OR clasific"&i&" LIKE '%62%' OR clasific"&i&" LIKE '%63%'" 
				next
				sql = sql & ") AND (nombre LIKE '%"&buscar&"%' OR sinonimos LIKE '%"&buscar&"%' OR cas LIKE '%"&buscar&"%' OR cee LIKE '%"&buscar&"%' OR rd LIKE '%"&buscar&"%')"
				...
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
				  ...
				  <tr>
				  	<td class="subtitulo3">
				  		<a href="risctox_tpr.asp?buscar=<%=buscar%>&ordenacion=nombre<%=texto_ord%>">Sustancias (<%=registros%>)</a>&nbsp;
				  	...
				  	<td class="subtitulo3">
				  		<a href="risctox_tpr.asp?buscar=<%=buscar%>&ordenacion=cas<%=texto_ord%>">CAS</a>&nbsp;
				  	...
				  	<td class="subtitulo3">
				  		<a href="risctox_tpr.asp?buscar=<%=buscar%>&ordenacion=cee<%=texto_ord%>">Nº CE</a>&nbsp;
				  	...
				  	<td class="subtitulo3">
				  		<a href="risctox_tpr.asp?buscar=<%=buscar%>&ordenacion=rd<%=texto_ord%>">RD 363/1995</a>&nbsp;
...
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="risctox_tpr.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="risctox_tpr.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
<%				end if %>				
				...
				<input type="hidden" name="titulo" value="Listado de sustancias tóxicas para la reproducción">
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='risctox_tpr.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
</script>