<% response.redirect "dn_risctox_res.asp" %>
<%
...
%>
			...
			<div id="texto">
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "<a href=risctox1.asp>Inicio</a>&nbsp;&gt;&nbsp;Sustancias que generan residuos peligrosos</p>"
              				   else
				...
				<form name="form_buscar" action="risctox_res.asp">
				<table class="tabla3" width="90%" align="center">
				  <tr>
					<td class="subtitulo3" nowrap align="right">Buscador de sustancias que generan residuos peligrosos:&nbsp;</td>
...
<%				buscar = request("buscar")
				...
				sql = "SELECT nombre,cas,cee,rd FROM RQ_SUSTANCIAS WHERE ((RD<>'' OR isnull(RD,'nulo')<>'nulo') AND (nombre LIKE '%"&buscar&"%' OR sinonimos LIKE '%"&buscar&"%' OR cas LIKE '%"&buscar&"%' OR cee LIKE '%"&buscar&"%' OR rd LIKE '%"&buscar&"%')) ORDER BY "&ordenacion
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
				  ...
				  <tr>
				  	<td class="subtitulo3">
				  		<a href="risctox_res.asp?buscar=<%=buscar%>&ordenacion=nombre<%=texto_ord%>">Sustancias (<%=registros%>)</a>&nbsp;
				  	...
				  	<td class="subtitulo3">
				  		<a href="risctox_res.asp?buscar=<%=buscar%>&ordenacion=cas<%=texto_ord%>">CAS</a>&nbsp;
				  	...
				  	<td class="subtitulo3">
				  		<a href="risctox_res.asp?buscar=<%=buscar%>&ordenacion=cee<%=texto_ord%>">Nº CE</a>&nbsp;
				  	...
				  	<td class="subtitulo3">
				  		<a href="risctox_res.asp?buscar=<%=buscar%>&ordenacion=rd<%=texto_ord%>">RD 363/1995</a>&nbsp;
...
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="risctox_res.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="risctox_res.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
<%				end if %>				
				...
				<input type="hidden" name="titulo" value="Listado de sustancias que generan residuos peligrosos">
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='risctox_res.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
</script>