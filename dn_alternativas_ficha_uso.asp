...
<%
	...
	set rstl=objConnection2.Execute("select nombre, descripcion from dn_risc_usos where id =" &id)
	nombre=h2(rstl("nombre"))

	descripcion=h2(rstl("descripcion"))
	rstl.close
	set rstl=nothing
%>
...
<td><p class=campo>Est&aacute;s en: <a href=index.asp?idpagina=550>Plataforma prevención de riesgo químico</a>&nbsp;&gt;&nbsp;<a href="dn_alternativas_portada.asp">BBDD Alternativas</a> &gt; Ficha de uso </p></td>
...
Listado de sustancias alternativas asociadas al uso, si las hay.  -->


<%
	set rstl=objConnection2.Execute("select dn_risc_sustancias.id, dn_risc_sustancias.nombre from dn_risc_sustancias_por_usos JOIN dn_risc_sustancias ON dn_risc_sustancias_por_usos.id_sustancia=dn_risc_sustancias.id where toxico=0 and id_uso =" &id)
	if rstl.eof then
		' response.write "No se encontraron sustancias alternativas"
...
<%
		...
		do while not rstl.eof
			response.write "<li><a href='dn_risctox_ficha_sustancia.asp?id_sustancia=" &rstl(0)& "'>" &rstl(1)& "</a></li>"
		  rstl.movenext
		loop
		response.write "</ul>"
		rstl.close
...
<%
	set rstl=objConnection2.Execute("select dn_alter_ficheros.id, dn_alter_ficheros.titulo, dn_alter_ficheros.num_alternativa from dn_alter_ficheros_por_usos JOIN dn_alter_ficheros ON dn_alter_ficheros_por_usos.id_fichero=dn_alter_ficheros.id where dn_alter_ficheros.tema<>'Casos prácticos' and dn_alter_ficheros_por_usos.id_uso =" &id&" ORDER BY dn_alter_ficheros.titulo")
	if rstl.eof then
...
<%
		...
		' response.write nums_alternativas
