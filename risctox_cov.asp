<% response.redirect "dn_risctox_cov.asp" %>
<%
	...
	if session("id_ecogente")="" then response.redirect "acceso.asp?pagina_esp=risctox_cov.asp"
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "<a href=risctox1.asp>Inicio</a>&nbsp;&gt;&nbsp;Compuestos orgánicos volátiles (COV)</p>"
              				   else
              				     response.write "<p class=campo>&nbsp;</p>"
              			   end if %>
				
				<p class=titulo3>RISCTOX: Compuestos orgánicos volátiles (COV)</p>

				<form name="form_buscar" action="risctox_cov.asp">
				<table class="tabla3" width="90%" align="center">
				  <tr>
					<td class="subtitulo3" nowrap align="right">Buscador de sustancias (COV):&nbsp;</td>
...
<%				buscar = request("buscar")
				...
				if request("ordenacion")="" then 
					ordenacion = "RQ_SUSTANCIAS_COV.nombre"
				else
					ordenacion = request("ordenacion")
				end if
				if right(ordenacion,4)<>"DESC" then texto_ord = " DESC"
					
				'sql = "SELECT RQ_SUSTANCIAS.*,RQ_SUSTANCIAS_COV.campo3 FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_COV ON RQ_SUSTANCIAS.CAS=RQ_SUSTANCIAS_COV.CAS "
				'sql = sql & "WHERE RQ_SUSTANCIAS.nombre<>'' AND (nombre LIKE '%"&buscar&"%' OR sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_COV.cas LIKE '%"&buscar&"%' OR cee LIKE '%"&buscar&"%' OR rd LIKE '%"&buscar&"%') ORDER BY "&ordenacion
				
				sql = "SELECT RQ_SUSTANCIAS.NOMBRE, RQ_SUSTANCIAS_COV.CAS, RQ_SUSTANCIAS_COV.NOMBRE AS SUSTANCIA "
				sql = sql & "FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_COV ON RQ_SUSTANCIAS.CAS = RQ_SUSTANCIAS_COV.CAS "
				if buscar<>"" then sql = sql & "WHERE (RQ_SUSTANCIAS_COV.sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_COV.nombre LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_COV.CAS LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.nombre LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.cas LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.cee LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.rd LIKE '%"&buscar&"%') "
				...
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
				  ...
				  <tr>
				  	<td class="subtitulo3">
				  		<a href="risctox_cov.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_COV.nombre<%=texto_ord%>">Sustancias (<%=registros%>)</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_COV.nombre DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_COV.nombre" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3">
				  		<a href="risctox_cov.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_COV.cas<%=texto_ord%>">CAS</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_COV.cas DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_COV.cas" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  </tr>
...
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="risctox_cov.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="risctox_cov.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
<%				end if %>				
				...
				<input type="hidden" name="titulo" value="Listado de sustancias (COV)">
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='risctox_cov.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
</script>
