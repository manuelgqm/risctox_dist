...
<%
	...
	'numeracion = "AICCA"
	idpagina = 626	'--- página Búsqueda, sólo para registrar estadísticas
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "<a href=risctox1.asp>Inicio</a>&nbsp;&gt;&nbsp;Resultado de buscar "&request("buscar")&"</p>"
              				   else
              				     response.write "<p class=campo>&nbsp;</p>"
              			   end if %>
				
				<p class=titulo3>Base de datos de sustancias tóxicas y peligrosas RISCTOX</p>

				<form name="form_buscar" action="risctox2.asp">
				<table class="tabla3" width="90%" align="center">
				  <tr>
					<td class="subtitulo3" nowrap align="right">Buscador de sustancias:&nbsp;</td>
				  ...
				  <tr>
					<td class="texto">&nbsp;</td>
					<td class="texto">(puedes escribir una parte del nombre o sinónimo,<br> o el número CAS, CE o RD)</td>
				  </tr>
				</table>
				</form>
...
<%				buscar = request("buscar")
				...
				sql = "SELECT * FROM RQ_SUSTANCIAS WHERE nombre LIKE '%"&buscar&"%' OR sinonimos LIKE '%"&buscar&"%' OR cas LIKE '%"&buscar&"%' OR cee LIKE '%"&buscar&"%' OR rd LIKE '%"&buscar&"%' ORDER BY "&ordenacion
				'response.write sql
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
				  ...
				  <tr>
				  	<td class="subtitulo3">
				  		<a href="risctox2.asp?buscar=<%=buscar%>&ordenacion=nombre<%=texto_ord%>">Sustancias (<%=registros%>)</a>&nbsp;
				  	...
				  	<td class="subtitulo3">
				  		<a href="risctox2.asp?buscar=<%=buscar%>&ordenacion=cas<%=texto_ord%>">CAS</a>&nbsp;
				  	...
				  	<td class="subtitulo3">
				  		<a href="risctox2.asp?buscar=<%=buscar%>&ordenacion=cee<%=texto_ord%>">Nº C.E./EINECS</a>&nbsp;
				  	...
				  	<td class="subtitulo3">
				  		<a href="risctox2.asp?buscar=<%=buscar%>&ordenacion=rd<%=texto_ord%>">R.D. 363/1995</a>&nbsp;
...
<%				objRecordset.movefirst
				...
				<tr>
					...
					<td class="celda_risctox" nowrap><% if instr(objRecordset("cas"),"-XX-")=0 then response.write formato(objRecordset("cas"),15)%>&nbsp;</td>
...
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="risctox2.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="risctox2.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
<%				end if %>				
				...
				<p align="center"><input type="button" class="boton" value="imprimir listado completo" onclick="print()"></p>
				</div>
				</div>
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='risctox2.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
</script>