<% response.redirect "dn_risctox_pyb.asp" %>
<%
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "<a href=risctox1.asp>Inicio</a>&nbsp;&gt;&nbsp;Sustancias tóxicas, persistentes y bioacumulativas</p>"
              				   else
              				     response.write "<p class=campo>&nbsp;</p>"
              			   end if %>
				
				<p class=titulo3>RISCTOX: Sustancias tóxicas, persistentes y bioacumulativas</p>

				<form name="form_buscar" action="risctox_pyb.asp">
				<table class="tabla3" width="90%" align="center">
				  <tr>
					<td class="subtitulo3" nowrap align="right">Buscador de sustancias tóxicas, persistentes y bioacumulativas:&nbsp;</td>
...
<%				buscar = request("buscar")
				...
				if request("ordenacion")="" then 
					ordenacion = "RQ_SUSTANCIAS_PYB.sustancia"
				else
					ordenacion = request("ordenacion")
				end if
				if right(ordenacion,4)<>"DESC" then texto_ord = " DESC"
					
				'sql = "SELECT RQ_SUSTANCIAS.*,RQ_SUSTANCIAS_PYB.campo3 FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_PYB ON RQ_SUSTANCIAS.CAS=RQ_SUSTANCIAS_PYB.CAS "
				'sql = sql & "WHERE RQ_SUSTANCIAS.nombre<>'' AND (nombre LIKE '%"&buscar&"%' OR sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_PYB.cas LIKE '%"&buscar&"%' OR cee LIKE '%"&buscar&"%' OR rd LIKE '%"&buscar&"%') ORDER BY "&ordenacion
				
				sql = "SELECT RQ_SUSTANCIAS.NOMBRE, RQ_SUSTANCIAS_PYB.CAS, RQ_SUSTANCIAS_PYB.sustancia, RQ_SUSTANCIAS_PYB.enlace, RQ_SUSTANCIAS_PYB.url "
				sql = sql & "FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_PYB ON RQ_SUSTANCIAS.CAS = RQ_SUSTANCIAS_PYB.CAS "
				sql = sql & "WHERE (sustancia LIKE '%"&buscar&"%' OR nombre LIKE '%"&buscar&"%' OR sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_PYB.cas LIKE '%"&buscar&"%' OR cee LIKE '%"&buscar&"%' OR rd LIKE '%"&buscar&"%') "
				...
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
				  ...
				  <tr>
				  	<td class="subtitulo3">
				  		<a href="risctox_pyb.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_PYB.sustancia<%=texto_ord%>">Sustancias (<%=registros%>)</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_PYB.sustancia DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_PYB.sustancia" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3">
				  		<a href="risctox_pyb.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_PYB.cas<%=texto_ord%>">CAS</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_PYB.cas DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_PYB.cas" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3">
				  		<a href="risctox_pyb.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_PYB.url<%=texto_ord%>">Más información (en inglés)</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_PYB.url DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_PYB.url" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  </tr>
<%				objRecordset.movefirst
				...
				<tr>
					...
					<td class="celda_risctox" nowrap><a href="<%=lcase(objRecordset("url"))%>" target="_blank"><%=mid(lcase(objRecordset("enlace")),1,20)%></a>&nbsp;</td>
				</tr>
<%				objRecordset.movenext
				reg = reg+1
				loop
...
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="risctox_pyb.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="risctox_pyb.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
<%				end if %>				
				...
				<input type="hidden" name="campo1" value="RQ_SUSTANCIAS_PYB.sustancia">
				<input type="hidden" name="campo2" value="RQ_SUSTANCIAS_PYB.cas">
				<input type="hidden" name="campo3" value="enlace">
				<input type="hidden" name="campo4" value="url">
				<input type="hidden" name="nombre1" value="nombre/sustancia">
				<input type="hidden" name="nombre2" value="cas">
				<input type="hidden" name="nombre3" value="más información (en inglés)">
				<input type="hidden" name="nombre4" value="url">
				<input type="hidden" name="sql" value="<%=sql%>">
				<input type="hidden" name="buscar" value="<%=buscar%>">
				<input type="hidden" name="titulo" value="Listado de sustancias tóxicas, persistentes y bioacumulativas">
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='risctox_pyb.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
</script>
