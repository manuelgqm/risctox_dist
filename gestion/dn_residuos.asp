...
if EliminaInyeccionSQL(request.Form("nombre"))<>"" then
	...
	codigo=h(EliminaInyeccionSQL(request.Form("codigo")))
	nombre=h(EliminaInyeccionSQL(request.Form("nombre")))
	
	sqls="insert into  rq_residuos (codigo,nombre) VALUES ('" &codigo& "', '" &nombre& "')"
	...
	case 1: 'han dado a buscar
		...
		sqls="select id from rq_residuos order by " &ordenacion& " " &sentido 
		'response.write sqls
	...
	if hr>0 then
		...
		sqlpag="select id,codigo,nombre from rq_residuos ORDER BY " &ordenacion&  " " &sentido
		...
		if not rstpag.eof then
			...
			if hr>0 then
				for contadorFilas=registroini to registrofin					
						  ...
						  tablares=tablares & "<td><a href='#' onclick=" &chr(34)& "abreVentanaCentrada('dn_asociaciones_mostrar.asp?tipo=residuo&id=" &arrayDatos(0,contadorFilas)& "', 800, 580); return false;" &chr(34)& "	><img src='imagenes/relacion.png' align='left' alt='Ver relación' /></a>&nbsp;" &arrayDatos(2,contadorFilas)& "</td>"		
						  
						  tablares=tablares & "<td align='center'><a href='#' onclick=" &chr(34)& "abreVentanaCentrada('dn_mod.asp?asociar=residuo&id=" &arrayDatos(0,contadorFilas)& "', 800, 300); return false;" &chr(34)& "	><img src='imagenes/icono_modificar.gif' alt='Modificar'></a></td>"
						

              'tablares=tablares & "<td align='center'><a href='#' onclick=" &chr(34)& "abreVentanaCentrada('dn_desasociar.asp?asociar=fich_residuo&id=" &arrayDatos(0,contadorFilas)& "', 800, 580); return false;" &chr(34)& "	><img src='imagenes/icono_estructura.gif' alt='Ficheros asociados' /></a></td>"
						 				
						  tablares=tablares & "</tr>" 
					'end if				
				next		
		
				'tablares="<table id='resultados' cellspacing='0' cellpadding='3' border='1' align='center'><tr><th><input type='checkbox' name='selector' onchange='cambiachecks(this)' /></th><th>Nº</th><th nowrap>CÓDIGO <a href='dn_residuos.asp?ordenacion=codigo&sentido=ASC' style='text-decoration:none'>&uarr;</a> <a href='dn_residuos.asp?ordenacion=codigo&sentido=DESC' style='text-decoration:none'>&darr;</a></th><th>RESIDUO <a href='dn_residuos.asp?ordenacion=nombre&sentido=ASC' style='text-decoration:none'>&uarr;</a> <a href='dn_residuos.asp?ordenacion=nombre&sentido=DESC' style='text-decoration:none'>&darr;</a></th><th>Modificar</th><th>Ver/Modificar Ficheros asociados</th></tr>" &tablares& "</table>" 
				tablares="<table id='resultados' cellspacing='0' cellpadding='3' border='1' align='center'><tr><th><input type='checkbox' name='selector' onchange='cambiachecks(this)' /></th><th>Nº</th><th nowrap>CÓDIGO <a href='dn_residuos.asp?ordenacion=codigo&sentido=ASC' style='text-decoration:none'>&uarr;</a> <a href='dn_residuos.asp?ordenacion=codigo&sentido=DESC' style='text-decoration:none'>&darr;</a></th><th>RESIDUO <a href='dn_residuos.asp?ordenacion=nombre&sentido=ASC' style='text-decoration:none'>&uarr;</a> <a href='dn_residuos.asp?ordenacion=nombre&sentido=DESC' style='text-decoration:none'>&darr;</a></th><th>Modificar</th></tr>" &tablares& "</table>" 
			end if
...
<h1>Alternativas / Residuos </h1>
<%flashMsgShow()%>

<form action="dn_residuos.asp" method="post" name="myform">
...
%>		
		...
		La acción sobre la que pulse se llevará a cabo sobre los residuos marcados 
		<input type="checkbox" name="ejemplo" checked />
		. 
		<div align="center" class='margengr'>
		<input type="button" onClick="eliminar('residuos');" value="ELIMINAR" />  
		<p>Asociar a: <input type="button" onClick="asociar('fich_residuo');" value="Ficheros" /> 
		</p>
	    </div>
		</fieldset>
...
<form action="dn_residuos.asp" method="post" name="myform2">
		<fieldset class="margengr centcontenido"><legend><strong>Nuevo residuo</strong></legend>
		<table align="center">
		<tr><td>Código</td><td align="left"> <input type="text" name="codigo" maxlength="10" size="10" /> </td></tr>
		<tr><td>Residuo</td><td> <input type="text" name="nombre" maxlength="2000" size="100" /> </td></tr>
		</table>
