...
sql = "select * from temporal_prohibidas where (prohibido IS NULL) and cas>'92045-17-5' order by cas"
...
while not objr.eof
	...
	if trim(objr("cas"))<>"" then
		sql2 = "select id, comentarios from dn_risc_sustancias where num_cas='"&cas&"' and num_cas<>''"
		...
		if not objr2.eof then
			...
			sql3 = "INSERT INTO dn_risc_sustancias_restringidas (id_sustancia,comentario_restringida) values ('"&id_sustancia&"','"&comentario&"')"
