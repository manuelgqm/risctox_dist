<%
	...
	'numeracion = "AICDA"
	idpagina = 632	'--- página Procesos (sólo para registrar estadísticas)
...
<head>
<title>ECOinformas: Base de datos de sustancias tóxicas y peligrosas RISCTOXresponse.write numeracion</title>
...
<%				registrosporpagina = 25
				...
				sql = "SELECT * FROM RQ_PROCESOS ORDER BY proceso"
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
		   	   	...
		   	   	if registros=0 then 
		   	   		...
		   	   		response.write "<p class=texto align=center>Pulsa sobre el nombre del proceso para ver las alternativas relacionadas:</p>"
		   	   		response.write "<table class=tabla3 width='90%' align=center border=0 cellpadding=4 cellspacing=0>"
		   	   	%>
				  <tr>
				  	<td class="subtitulo3">
				  		<table width="100%" align="center"><tr><td>Procesos (<%=registros%>)&nbsp;</td>
				  		<td align=right><img src="imagenes/ico_alt_procesos.gif"></td></tr></table>
				  	</td>
				  </tr>
<%				objRecordset.movefirst
				...
				<tr>
					<td class="celda_risctox"><a href="alternativas.asp?proceso=<%=objRecordset("idproceso")%>&nombre=<%=objRecordset("proceso")%>" title="<%=objRecordset("proceso")%>"><%=formato(objRecordset("proceso"),300)%></a>&nbsp;</td>
				</tr>
<%				objRecordset.movenext
				i = i+1
				loop
...
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="procesos.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="procesos.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='procesos.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
</script>