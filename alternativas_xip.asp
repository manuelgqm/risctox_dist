<%
	...
	'numeracion = "AICDA"
	idpagina = 634	'--- página Listado alternativas (sólo para registrar estadísticas)
...
<%				registrosporpagina = 25
				...
				sql = "SELECT * FROM RQ_ALTERNATIVAS ORDER BY idalternativa"
				
				if request("sector")<>"" then sql = "SELECT RQ_ALTERNATIVAS.alternativa,RQ_ALTERNATIVAS.idalternativa FROM RQ_ALTERNATIVAS LEFT JOIN RQ_ALTERNATIVAS_RELACIONES ON RQ_ALTERNATIVAS.idalternativa=RQ_ALTERNATIVAS_RELACIONES.idalternativa WHERE RQ_ALTERNATIVAS_RELACIONES.tabla_relacion='RQ_SECTORES' AND RQ_ALTERNATIVAS_RELACIONES.id_relacion="&request("sector")&" ORDER BY RQ_ALTERNATIVAS.idalternativa"
				if request("proceso")<>"" then sql = "SELECT RQ_ALTERNATIVAS.alternativa,RQ_ALTERNATIVAS.idalternativa FROM RQ_ALTERNATIVAS LEFT JOIN RQ_ALTERNATIVAS_RELACIONES ON RQ_ALTERNATIVAS.idalternativa=RQ_ALTERNATIVAS_RELACIONES.idalternativa WHERE RQ_ALTERNATIVAS_RELACIONES.tabla_relacion='RQ_PROCESOS' AND RQ_ALTERNATIVAS_RELACIONES.id_relacion="&request("proceso")&" ORDER BY RQ_ALTERNATIVAS.idalternativa"
				if request("producto")<>"" then sql = "SELECT RQ_ALTERNATIVAS.alternativa,RQ_ALTERNATIVAS.idalternativa FROM RQ_ALTERNATIVAS LEFT JOIN RQ_ALTERNATIVAS_RELACIONES ON RQ_ALTERNATIVAS.idalternativa=RQ_ALTERNATIVAS_RELACIONES.idalternativa WHERE RQ_ALTERNATIVAS_RELACIONES.tabla_relacion='RQ_PRODUCTOS' AND RQ_ALTERNATIVAS_RELACIONES.id_relacion="&request("producto")&" ORDER BY RQ_ALTERNATIVAS.idalternativa"
				'response.write sql
				'if 1=0 then
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
		   	   	...
		   	   	if registros=0 then 
		   	   		...
		   	   		response.write "<tr><td class=texto colspan=4>No hay alternativas</td></tr>"
		   	   	else 
		   	   		response.write "<p class=texto align=center>Pulsa sobre el nombre de la alternativa para ver toda su información:</p>"
		   	   		response.write "<table class=tabla3 width='90%' align=center border=0 cellpadding=4 cellspacing=0>"
		   	   	%>
				  <tr>
				  	<td class="subtitulo3">
				  		<table width="100%" align="center"><tr><td>Alternativas (<%=registros%>)&nbsp;
				  		<% if request("sector")<>"" then response.write "<br>para el sector: "&request("nombre") %>
				  		<% if request("proceso")<>"" then response.write "<br>para el proceso: "&request("nombre") %>
				  		<% if request("producto")<>"" then response.write "<br>para el producto: "&request("nombre") %>
				  		</td>
				  		<td align=right><img src="imagenes/ico_alt_alternativas.gif"></td></tr></table>
				  	</td>
				  </tr>
<%				objRecordset.movefirst
				...
				<tr>
					<td class="celda_risctox"><a href="alternativa.asp?id=<%=objRecordset("idalternativa")%>" title="<%=objRecordset("alternativa")%>"><%=formato(objRecordset("alternativa"),300)%></a>&nbsp;</td>
				</tr>
<%				objRecordset.movenext
				i = i+1
				loop
...
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="alternativas.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="alternativas.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='alternativas.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
</script>
<% 'end if %>