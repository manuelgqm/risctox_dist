<%
	...
	'numeracion = "AICDA"
	idpagina = 631	'--- página Productos (sólo para registrar estadísticas)
...
<%				registrosporpagina = 25
				...
				sql = "SELECT * FROM RQ_PRODUCTOS ORDER BY producto"
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
		   	   	...
		   	   	if registros=0 then 
		   	   		...
		   	   		response.write "<p class=texto align=center>Pulsa sobre el nombre del producto para ver las alternativas relacionadas:</p>"
		   	   		response.write "<table class=tabla3 width='90%' align=center border=0 cellpadding=4 cellspacing=0>"
		   	   	%>
				  <tr>
				  	<td class="subtitulo3">
				  		<table width="100%" align="center"><tr><td>Productos (<%=registros%>)&nbsp;</td>
				  		<td align=right><img src="imagenes/ico_alt_productos.gif"></td></tr></table>
				  	</td>
				  </tr>
<%				objRecordset.movefirst
				...
				<tr>
					<td class="celda_risctox"><a href="alternativas.asp?producto=<%=objRecordset("idproducto")%>&nombre=<%=objRecordset("producto")%>" title="<%=objRecordset("producto")%>"><%=formato(objRecordset("producto"),300)%></a>&nbsp;</td>
				</tr>
<%				objRecordset.movenext
				i = i+1
				loop
...
<% 				if objRecordset.Pagecount>1 then%>
				<tr><td class=texto colspan=4 align="center">Hay <%=registros%> productos que cumplen los criterios de búsqueda. Se muestran sólo <%=registrosporpagina%> por página.</td></tr>
				<tr><td class=texto colspan=4 align="center">
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="productos.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="productos.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='productos.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
</script>