...
<%
	...
	sql = "SELECT titulo,numeracion,pagina,fecha,hora,autor,tipo,visible,destino, titulo_eng, pagina_eng, fecha_modificacion FROM WEBISTAS_PAGINAS WHERE idpagina="&id
...
<% for i1=1 to 10
	valor = ""
	if len(numeracion)>=i1 then 
		valor = cstr(asc(mid(numeracion,i1,1))-64)
		if i1>1 then numeracioncompleta = numeracioncompleta&valor&"."
	end if
%>
<%	if i1<3 then %>
<input type="hidden" name="clasif<%=i1%>" value="<%=valor%>" size="2" class="campo" maxlength="2">
<%	else %>
<input type="text" name="clasif<%=i1%>" value="<%=valor%>" size="2" class="campo" maxlength="2">.&nbsp;
<%	end if %>
<% next %>
&nbsp;<input type="button" class="boton" value="DESPLAZAR" onClick="mover()">
<br>
<%	titulocompleto = ""
	for i2=2 to len(numeracion)
		sql = "SELECT titulo,numeracion,idpagina FROM WEBISTAS_PAGINAS WHERE numeracion='" & mid(numeracion,1,i2) & "'"
		set objRecordset = Server.CreateObject ("ADODB.Recordset")
		set objRecordset = OBJConnection.Execute(sql)
		if i2<>2 then titulocompleto = titulocompleto & "&nbsp;&gt;&nbsp;" 
...
<option value="1" <%if visible=1 then response.write "selected"%>>Sí</option>
...
<table align="center"><tr><td><input type="button" value="GUARDAR CAMBIOS" class="boton" onClick="document.formulario.submit()"></td></tr></table>
<br>
