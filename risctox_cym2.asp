<% response.redirect "dn_risctox_cym2.asp" %>
<%
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "<a href=risctox1.asp>Inicio</a>&nbsp;&gt;&nbsp;Sustancias cancerígenas o mutágenas (según IARC)</p>"
              				   else
              				     response.write "<p class=campo>&nbsp;</p>"
              			   end if %>
				
				<p class=titulo3>RISCTOX: Sustancias cancerígenas o mutágenas (según IARC)</p>

				<form name="form_buscar" action="risctox_cym2.asp">
				<table class="tabla3" width="90%" align="center">
				  <tr>
					<td class="subtitulo3" nowrap align="right">Buscador de sustancias cancerígenas o mutágenas:&nbsp;</td>
...
<%				buscar = request("buscar")
				...
				if request("ordenacion")="" then 
					ordenacion = "RQ_SUSTANCIAS_CYM2.nombre"
				else
					ordenacion = request("ordenacion")
				end if
				if right(ordenacion,4)<>"DESC" then texto_ord = " DESC"
					
				'sql = "SELECT RQ_SUSTANCIAS.*,RQ_SUSTANCIAS_CYM2.grupo,RQ_SUSTANCIAS_CYM2.volumen FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_CYM2 ON RQ_SUSTANCIAS.CAS=RQ_SUSTANCIAS_CYM2.CAS "
				'sql = sql & "WHERE RQ_SUSTANCIAS.nombre<>'' AND (nombre LIKE '%"&buscar&"%' OR sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_CYM2.cas LIKE '%"&buscar&"%' OR cee LIKE '%"&buscar&"%' OR rd LIKE '%"&buscar&"%') ORDER BY "&ordenacion
				
				sql = "SELECT RQ_SUSTANCIAS.NOMBRE, RQ_SUSTANCIAS_CYM2.CAS, RQ_SUSTANCIAS_CYM2.nombre as SUSTANCIA, RQ_SUSTANCIAS_CYM2.grupo, RQ_SUSTANCIAS_CYM2.volumen FROM RQ_SUSTANCIAS RIGHT JOIN RQ_SUSTANCIAS_CYM2 ON RQ_SUSTANCIAS.CAS=RQ_SUSTANCIAS_CYM2.CAS "
				if buscar<>"" then sql = sql & "WHERE (RQ_SUSTANCIAS_CYM2.CAS LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS_CYM2.nombre LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.nombre LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.sinonimos LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.cas LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.cee LIKE '%"&buscar&"%' OR RQ_SUSTANCIAS.rd LIKE '%"&buscar&"%') "
				...
				set objRecordset = Server.CreateObject ("ADODB.Recordset")
				  ...
				  <tr>
				  	<td class="subtitulo3">
				  		<a href="risctox_cym2.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_CYM2.nombre<%=texto_ord%>">Sustancias (<%=registros%>)</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM2.nombre DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM2.nombre" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3">
				  		<a href="risctox_cym2.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_CYM2.cas<%=texto_ord%>">CAS</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM2.cas DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM2.cas" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3">
				  		<a href="risctox_cym2.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_CYM2.grupo<%=texto_ord%>">GRUPO</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM2.grupo DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM2.grupo" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  	<td class="subtitulo3">
				  		<a href="risctox_cym2.asp?buscar=<%=buscar%>&ordenacion=RQ_SUSTANCIAS_CYM2.volumen<%=texto_ord%>">VOLUMEN</a>&nbsp;
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM2.volumen DESC" then response.write "<img src=imagenes/abajo.gif>" %>
				  		<% if ordenacion="RQ_SUSTANCIAS_CYM2.volumen" then response.write "<img src=imagenes/arriba.gif>" %>
				  	</td>
				  </tr>
<%				objRecordset.movefirst
				...
				<tr>
					...
					<% if trim(ucase(objRecordset("grupo")))="GRUPO 1" then 
					   ...
					   if trim(ucase(objRecordset("grupo")))="GRUPO 2A" then 
					   ...
					   if trim(ucase(objRecordset("grupo")))="GRUPO 2B" then 
					   ...
					   if trim(ucase(objRecordset("grupo")))="GRUPO 3" then 
					   ...
					   if trim(ucase(objRecordset("grupo")))="GRUPO 4" then 
					...
					<td class="celda_risctox"><%=formato(objRecordset("volumen"),20)%>&nbsp;</td>
				</tr>
<%				objRecordset.movenext
				i = i+1
				loop
...
<%				if Clng(Session("pagina")) > 1 then %>
				<a href="risctox_cym2.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))-1%>">anterior&nbsp;&lt;&lt;</a>&nbsp;&nbsp;&nbsp;
<%				end if %>
				Página&nbsp;<%=Session("pagina")%>&nbsp;de&nbsp;<%=objRecordset.Pagecount%>
<%				if Clng(Session("pagina")) < objRecordset.Pagecount then %>
				&nbsp;&nbsp;&nbsp;<a href="risctox_cym2.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag=<%=Clng(Session("pagina"))+1%>">&gt;&gt;&nbsp;siguiente</a>
<%				end if %>
...
<%				end if %>				
				...
				<input type="hidden" name="campo1" value="RQ_SUSTANCIAS_CYM2.nombre">
				<input type="hidden" name="campo2" value="RQ_SUSTANCIAS_CYM2.cas">
				<input type="hidden" name="campo3" value="grupo">
				<input type="hidden" name="campo4" value="volumen">
				<input type="hidden" name="nombre1" value="nombre/sustancia">
				<input type="hidden" name="nombre2" value="cas">
				<input type="hidden" name="nombre3" value="grupo">
				<input type="hidden" name="nombre4" value="volumen">
				<input type="hidden" name="sql" value="<%=sql%>">
				<input type="hidden" name="buscar" value="<%=buscar%>">
				<input type="hidden" name="titulo" value="Listado de sustancias cancerígenas o mutágenas (según IARC)">
...
{	if (form_pag.pag_ir.value><%=objRecordset.Pagecount%>)
	...
	{ location.href='risctox_cym2.asp?ordenacion=<%=ordenacion%>&buscar=<%=buscar%>&pag='+form_pag.pag_ir.value; }
}
</script>
