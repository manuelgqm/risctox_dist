<%
	...
	idpagina = 1178	'--- página buscador del Centro de dcumentación (sólo para registrar estadísticas)
	'----- Registrar la visita
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "Resultado de la búsqueda</p>"
              				   else
...
<p class=texto"><b>Hay <%=num_resultados%> páginas.<% if num_resultados>0 then %> Pulsa sobre una de ellas para visitarla<%else%> Prueba con otra palabra<% end if %></b></p>
...
<%   					     objRecordset.movenext
			   		     loop %>
						<p align=center class="tabla2"><img src='imagenes/candado.gif' border=0 alt='Página restringida'>&nbsp;Páginas restringidas. Se requiere clave para acceder que se puede solicitar pulsando <a href="formulario2.asp">aquí</a></p>			   		     
			   		 <%  else %>
			   		   <p class="texto">No hay ninguna página coincidente con esta búsqueda</p>
  					 <p>&nbsp;</p>
					   <p>&nbsp;</p>
