<%
	...
	if session("id_ecogente")="" then response.redirect "acceso.asp?idpagina=572"
	
	numeracion = "AIBCEA"
	seccion = asc(mid(numeracion,3,1))-64

	idpagina = 673	'--- página inicio Enlaces (sólo para registrar estadísticas)
	'----- Registrar la visita
				...
				<div class="texto">
             				<% if len(numeracion)>3 then
			   	   	   	...
			   	   	   	set objRecordset = Server.CreateObject ("ADODB.Recordset")
              				     ...
              				     next
              				     response.write "<a href=index.asp?idpagina=572>Enlaces</a>&nbsp;&gt;&nbsp;Inicio</p>"
              				   else
					...
					<% sql = "SELECT id,nombre FROM ENL_TEMAS WHERE numeracion LIKE '" & numeracion & "%' AND numeracion<>'" & numeracion & "' ORDER BY numeracion"
					   set objRecordset = Server.CreateObject ("ADODB.Recordset")
			   		   set objRecordset = OBJConnection.Execute(sql)
			   		   do while not objRecordset.eof %>
					<tr><td class="texto"><img src="imagenes/ico_industria.gif" border="0" align="absmiddle">&nbsp;<a href="enlaces2.asp?idtipo=<%=objRecordset("id")%>"><%=ucase(objRecordset("nombre"))%></a></td></tr>
					<% objRecordset.movenext
					   loop %>
					</table>
					...
					<p class="titulo2">Elige el grupo de enlaces de interés que quieres consultar:</p>
					<table align="center" width="95%" cellpadding=0 cellspacing=5 border=0 style="background-color: #EFEFEF;" class="tabla2"><tr><td>
					<table style="background: url(imagenes/buscador.gif); background-repeat: no-repeat; background-position: top left; color: #EFEFEF;"><tr><td>
					<table width="95%" cellpadding=0 cellspacing=5 border=0>
