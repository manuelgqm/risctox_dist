<%
	...
	idpagina = 655	'--- página listado enlaces Vig. Tecnológica (sólo para registrar estadísticas)
	'----- Registrar la visita
...
%>
			...
			<% end if %>
				...
				<div class="texto">
					...
					<p class="titulo2">Resultados de la búsqueda en la base de datos de vigilancia tecnólogica:</p>
					...
					<tr><td class="texto" valign="top" align="right"><a href="abreenlacer.asp?idenlace=<%=objRecordset("Id")%>" target="_blank"><img src="imagenes/ico_puntito.gif" valign="top" border=0></a></td><td class="texto" bgcolor="#<%=color%>"><b><%=objRecordset("afiliacion")%></b><br><a href="abreenlacer.asp?idenlace=<%=objRecordset("Id")%>" target="_blank"><%=objRecordset("titulo")%><br><a href="abreenlacer.asp?idenlace=<%=objRecordset("Id")%>" target="_blank"><%=objRecordset("url")%>
					<% sql2 = "SELECT indice FROM ENL_ENLACES WHERE id="&objRecordset("id")
					   set objRecordset2 = Server.CreateObject ("ADODB.Recordset")
					   objRecordset2.Open sql2,objConnection,adOpenKeyset
					   if not isnull(objRecordset2("indice")) and objRecordset2("indice")<>"" then %>
